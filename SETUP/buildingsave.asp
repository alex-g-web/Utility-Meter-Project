...
dim bldg, bldgname, address, alertOn, city, state, zip, sqft, action, pid, region, facilityType, btbldgname, btstrt, btcity, btstate, btzip, Submeters15, Submeters1, offline, ContactName, ContactPhone
...
elseif trim(action)="Delete" then
	...
	strsql = "UPDATE buildings set bldgname='"&bldgname&"', alertOn = '"&alertOn&"', strt='"&address&"', city='"&city&"', state='"&state&"', zip='"&zip&"', sqft='"&sqft&"', region='"&region&"', facilityType='"&facilityType&"', btbldgname='"&btbldgname&"', btstrt='"&btstrt&"', btcity='"&btcity&"', btstate='"&btstate&"', btzip='"&btzip&"', offline="&offline&", ContactName='"&ContactName&"', ContactPhone='"&ContactPhone&"' WHERE bldgnum='"&bldg&"'"
end if
'response.Write strsql
'response.End
...
if trim(action)="Save" then 'need to find the bid for the building just added
	...
	if not rst1.eof then portfolio = rst1("portfolio")
	rst1.close
	dim fs,f, fdir, portfolio
	fdir = "\\serv-pdf\pdfmaker\"&ucase(portfolio)&"\"&ucase(bldg)
