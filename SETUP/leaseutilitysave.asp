...
dim AdminFee, TenantRate, AddonFee, ModifyRate, Coincident, CoinWithPeak, FullOnPeak, utility, procname, calcintpeak, acctidref, use_acctid, billnote, shadow, externalfee, externalfeename, lmepcredit
dim UseBldgRate
UseBldgRate = request("UseBldgRate")
...
externalfee = trim(request("externalfee"))
externalfeename = trim(request("externalfeename"))
lmepcredit=trim(request("lmepcredit"))
...
if trim(externalfee)=""  then externalfee=0
if trim(lmepcredit)="" then lmepcredit=0

if trim(action)="Save" then
	strsql = "INSERT INTO tblleasesutilityprices (AdminFee, RateTenant, AddonFee, RateModify, Coincident, Coincident_peak, FullOnPeak,billingid,utility, procname, calcintpeak, acctid, use_acctid, bill_note, shadow, UseBldgRate, externalfee, externalfeename, lmepcredit) values ('"&AdminFee&"', '"&TenantRate&"', "&AddonFee&", '"&ModifyRate&"', '"&Coincident&"', '"&CoinWithPeak&"', '"&FullOnPeak&"', '"&tid&"', '"&utility&"', '"&procname&"', '"&calcintpeak&"', '"&acctidref&"', "&use_acctid&", '"&billnote&"', '"&shadow&"', '"&UseBldgRate&"', '"&externalfee&"', '"&externalfeename&"', '"&lmepcredit&"')"
elseif trim(action)="Delete" then
	strsql = "DELETE FROM tblleasesutilityprices WHERE leaseutilityid="&lid
else
	strsql = "UPDATE tblleasesutilityprices set AdminFee='"&AdminFee&"', RateTenant='"&TenantRate&"', AddonFee="&AddonFee&", RateModify='"&ModifyRate&"', Coincident='"&Coincident&"', Coincident_peak='"&CoinWithPeak&"', FullOnPeak='"&FullOnPeak&"', billingid='"&tid&"', utility='"&utility&"', procname='"&procname&"', calcintpeak='"&calcintpeak&"', acctid='"&acctidref&"', use_acctid='"&use_acctid&"', bill_note='"&billnote&"', shadow='"&shadow&"', UseBldgRate='"&UseBldgRate&"', externalfee='"&externalfee&"', externalfeename='"&externalfeename&"', lmepcredit='"&lmepcredit&"' WHERE leaseutilityid="&lid
end if
