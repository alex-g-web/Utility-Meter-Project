...
dim cnn1, rst1,rst5,rst6, strsql, ticketcount, masterticketid,opentickets, criticalopentickets, totaltickets
...
{
	cWin = window.open('/genergy2/billing/processbills.asp?pid=<%=pid%>','Process Bills','width=1200,height=1000,resizable=yes, scrollbars=yes')
	cWin.focus();
}
...
<tr bgcolor="#eeeeee">
  ...
  <!--****************added line of code and rst5 to accomodate to drop down selection to default to genergy basic. 8/11/2008 - Michelle T. ***************-->
   
 <!-- <%  rst5.open "select * from dbcore.dbo.billtemplates where name like '%gen%'",cnn1 %>
  <option value="<%=rst5("id")%>"<%if cint(billtemplate)="genergy basic" then response.write " SELECTED"%>><%="GEnergyAI"%></option>  
      rst5.close
 -->
  <%
      rst1.open "SELECT  * FROM dbcore.dbo.billtemplates where name not in ('genergy basic') order by id", cnn1
...
<tr bgcolor="#eeeeee">
  ...
  <tr>
	...
	<td width="20%">
    <%
'Response.Write("SELECT * FROM custom_links WHERE code=0 and unitid='"&pid&"'")
	...
	<td width="60%">&nbsp;
	<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="processbills(<%=pid%>);" class="mgmtlink">Process Bills</a>
	</td>
  </tr>
  </table>
	</td>
...
<%
  strsql = "SELECT b.bldgname,b.strt,b.city,b.state,b.zip,b.bldgnum, offline FROM buildings b left join regions on b.region=regions.id where portfolioid='" & pid & "' order by offline asc, bldgname asc"
  ...
  if not rst1.eof then %>
    ...
    <tr bgcolor="#eeeeee">
      ...
      <td colspan="2" align="right" style="border-top:1px solid #ffffff;border-bottom:1px solid #999999;"><input type="button" value="Add Building" onclick="buildingEdit('');" id=1 name=1 class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"></td>
		...
		<% do until rst1.eof 
			ccolor = ""
			if rst1("offline")=true then ccolor="class=""grayout"""
			%>
		<tr valign="top" bgcolor="#ffffff" onmouseover="this.style.backgroundColor = 'lightgreen'" onmouseout="this.style.backgroundColor = 'white'" onclick="buildingEdit('<%=rst1("bldgnum")%>');">
			...
			<td <%=ccolor%>><span class="standard"><%=ucase(rst1("bldgnum"))%></span></td>
		</tr>
		<% rst1.movenext
		loop %>
    </table>
    ...
    <tr bgcolor="#eeeeee">
      ...
      <td colspan="4" align="right" style="border-top:1px solid #ffffff; border-bottom:1px solid #999999;"></td>
		...
		<tr valign="top" bgcolor="#ffffff" onmouseover="this.style.backgroundColor = 'lightgreen'" onmouseout="this.style.backgroundColor = 'white'" onclick="buildingEdit('<%=rst1("bldgnum")%>');">
			...
			<%   'Response.Write("SELECT p.name FROM dbo.view_buildings b LEFT JOIN portfolio p ON p.id = newPid WHERE endDate IS NULL AND oldPid='" & pid & "' AND bldgNum ='" + rst1("bldgNum") + "'")
				'Response.End
...
'strsql2 = "SELECT p.name FROM dbo.view_buildings b LEFT JOIN portfolio p ON p.id = newPid WHERE endDate IS NULL AND bldgNum ='" + rst1("bldgNum") + "'" 
				'response.write strsql2 & "<br>"
				
				'response.end
			     rst6.open strsql2, cnn1
            %>
            <td <%=ccolor%>><span class="standard"><% if not rst6.EOF then%><%=rst6("name")%><% end if%></span></td>
            <% rst6.close %>
