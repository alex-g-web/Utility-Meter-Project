...
Dim minValue, maxValue, estimate, gatewayIp, gatewayDevice, gateComm, ipidAddress, modbus, readerNotes, hascoincd
...
if instr(trim(action),"Confirm")=0 and trim(online)<>"1" then%>
  ...
  <input type="hidden" name="GatewayDeviceId" value="<%=GatewayDeviceId%>"> 
   ...
   <input type="hidden" name="hascoincd" value="<%=hascoincd%>"> 
  </form>
  <%
  response.end
end if
...
'Update again
strsql = "update meters set hascoincd = '" & hascoincd & "' where LeaseUtilityId = (select leaseutilityid from meters where meterid = "&meterid&")"
cnn1.Execute strsql
