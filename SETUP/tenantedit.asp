...
dim tenantnum, flr, sqft, taxexempt, billingname, leaseexpired, interm, startdate, tName, tStrt, tCity, tState, tZip, customsrc, lmepExempt, onlinebill, ibsexempt, bsexempt, accounttype, email, paperless
dim corpStreet, corpCity, corpState, corpZip, tCountry, corpCountry
dim basebuilding
dim leasereviewno, leasereviewyes,tenantsetupyes,residential
...
dim rst4, TenantMoveInDate, LeaseExpirydate
set rst4 = server.createobject("ADODB.recordset")

if trim(bldg)<>"" then
  rst1.open "SELECT bldgname, b.strt, b.city, b.state, b.zip FROM buildings b WHERE bldgnum='"&bldg&"'", cnn1
...
'response.end
	if not rst1.EOF then
        ...
        basebuilding = rst1("basebuilding")
        tenantsetupyes = rst1("tenantsetupyes")
        leasereviewyes = rst1("leasereviewyes")
        residential = rst1("residential")
		interm = rst1("interm")
		tName = rst1("tName")
		tStrt = rst1("tStrt")
		tCity = rst1("tCity")
		...
		email = rst1("paperless_email")
		paperless = rst1("paperless")
		...
		rst4.Open "SELECT BillingId, MoveInDate FROM tblTenantExtDetails WHERE BillingId = '" & tid & "'", cnn1
...
<form name="form2" method="post" action="tenantsave.asp">
  <table width="100%" border="0" cellpadding="3" cellspacing="0">
    <tr bgcolor="#6699cc"> 
      <td height="26"> <table border=0 cellpadding="0" cellspacing="0" width="100%">
          <tr> 
            <td> <span class="standardheader"> 
              ...
              View
              <% end if %>
              Account | <span style="font-weight:normal;"><nobr><a href="portfolioedit.asp?pid=<%=pid%>" style="color:#ffffff;"><%=portfolioname%></a></nobr> 
              &gt; <nobr><a href="buildingedit.asp?pid=<%=pid%>&bldg=<%=bldg%>" style="color:#ffffff;"><%=bldgname%></a></nobr> 
      ...
      <td align="right"><%if trim(tid)<>"" and not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0) then ticket.MakeButton%> 
    ...
    <tr bgcolor="#eeeeee" valign="top"> 
      <td colspan="2"> 
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr>
            <td valign="top"> <table border="0" cellpadding="3" cellspacing="0" width="100%">
                ...
                <tr>
                  ...
                  <td>
					<input type="text" name="paperless_email" value="<%=email%>"/>
                  </td>
                </tr>				
                <tr>
            ...
            <td valign="top"> <table border="0" cellpadding="3" cellspacing="0" width="100%">
                ...
                <tr>
                  <td align="right">Base Building</td>
                  <td><input type="checkbox" value="1" name="basebuilding" <%if basebuilding="True" then Response.Write "CHECKED"%>></td>
                </tr>
                <tr>
                  <td align="right">Tenant Setup Charge</td>
                  <td><input type="checkbox" value="1" name="tenantsetupyes" <%if tenantsetupyes="True" then Response.Write "CHECKED"%>></td>
                </tr>
                <tr>
                  <td align="right">Lease Review Charge</td>
                  <td><input type="checkbox" value="1" name="leasereviewyes" <%if leasereviewyes="True" then Response.Write "CHECKED"%>></td>
                </tr>
                <tr>
                  <td align="right">Residential</td>
                  <td><input type="checkbox" value="1" name="residential" <%if residential="True" then Response.Write "CHECKED"%>></td>
                </tr>
                <tr>
          ...
          <tr>
            <td colpsan="2"> 
			<%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%>
              <%if trim(tid)<>"" then%>
              <input type="submit" name="action" value="Update" class="standard" style="border:1px outset #ddffdd;background-color:ccf3cc;"> 
              <input type="button" name="action" value="Cancel" onclick="document.location='tenantedit.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>&edit=0';" class="standard" style="border:1px outset #ddffdd;background-color:ccf3cc;"> 
              &nbsp; 
              <%else%>
		...
		<% if criticalopentickets <> 0 then %>
		  <tr align="center"> 
            <td colspan =2 bgcolor="#FF0000"><b><a href="#" onclick="window.open('/genergy2_intranet/itservices/ttracker/troublesearch.asp?searchstring=<%=tid%>&action=Search&searchbox=True&accounts=True','SearchNotes','width=800,height=400, scrollbars=no')"><%=criticalopentickets%> 
              Open Ticket(s) for this account that will effect Reading and Billing</a></b> 
            </td>
          </tr>		
		<% end if %>
          <tr> 
            ...
            <tr> 
              <td> 
                ...
                <table border=0 cellpadding="3" cellspacing="0">
                  ...
                  <tr valign="top" bgcolor="#eeeeee"> 
                    <td><span class="standard">Email Address:</span></br><% if paperless then %><img src="images/paperless.png" width="24" height="24" border="0"> <% end if %></td>
                    <td> <%=email%></td>
                  </tr>				  
                  <% if corpStreet <> "" then %>
                  <tr valign="top" bgcolor="#eeeeee"> 
              ...
              <td width="49%"> 
                ...
                <table border=0 cellpadding="3" cellspacing="0">
                  ...
                  <tr valign="top" bgcolor="#eeeeee"> 
                    <td><span class="standard">Base Building:</span></td>
                    <td> 
                      <%if basebuilding="True" then Response.Write "Yes" else Response.Write "No" end if%>
                    </td>
                  </tr>
                  <tr valign="top" bgcolor="#eeeeee"> 
                    <td><span class="standard">Tenant Setup Charge:</span></td>
                    <td> 
                      <%if tenantsetupyes="True" then Response.Write "Yes" else Response.Write "No" end if%>
                    </td>
                  </tr>
                  <tr valign="top" bgcolor="#eeeeee"> 
                    <td><span class="standard">Lease Review Charge:</span></td>
                    <td> 
                      <%if leasereviewyes="True" then Response.Write "Yes" else Response.Write "No" end if%>
                    </td>
                  </tr>
                  <tr valign="top" bgcolor="#eeeeee"> 
                    <td><span class="standard">Residential:</span></td>
                    <td> 
                      <%if residential="True" then Response.Write "Yes" else Response.Write "No" end if%>
                    </td>
                  </tr>
                  <!-- onlinebill -->
                  <tr valign="top" bgcolor="#eeeeee"> 
        ...
        <%if trim(tid)<>"" and not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0) then%>
        <input type="button" name="action" value="Edit Account Information" onclick="document.location='tenantedit.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>&edit=1';" class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;margin-top:3px;"> 
        <input type="submit" name="action" value="Transfer Info To New Account" class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;margin-top:3px;"> 
        <%end if%>
        <% end if %>
      </td>
    </tr>
    <%if trim(tid)<>"" then%>
    <tr valign="top" bgcolor="#eeeeee"> 
      <td colspan="2" style="border-bottom:1px solid #999999"> <table border=0 cellpadding="3" cellspacing="0">
          <tr valign="top"> 
            <td> 
              <!-- begin core component links -->
              <img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="javascript:document.location='groupView.asp?pid=<%=pid%>&bldg=<%=bldg%>&tid=<%=tid%>'" class="mgmtlink">Manage 
              account groups</a> 
...
rst1.open "SELECT * FROM custom_links WHERE code=2 and unitid='"&pid&"'", cnn1
dim customlinks
customlinks = false
if not rst1.eof then
  customlinks = true
...
do while not rst1.eof
	response.write "<img src=""images/aro-rt.gif"" align=""absmiddle"" hspace=""2"" border=""0""><a href=""javascript:openCustomWin('"&rst1("link")&"?pid="&pid&"&bldg="&bldg&"&tid="&tid&"','customlink', 'width="&rst1("width")&",height="&rst1("height")&"');"" class=""custlink"">"&rst1("label")&"</a><br>"
	rst1.movenext
loop
  end if
%>
...
<table width="100%" border=0 cellpadding="3" cellspacing="0">
<% if not(edit) and trim(tid)<>"" then %>
    <tr bgcolor="#eeeeee">
      <td valign="middle" align="left" colspan="5" style="border-bottom:1px solid #999999"><%ticket.Display pid,true, true, true%></td>
    </tr>
	<%end if %>
<%
if trim(tid)<>"" then
...
<tr valign="top" bgcolor="#eeeeee">
  ...
  <% do until rst1.EOF %>
  Jump to:&nbsp;<a href="#<%=rst1("utilitydisplay")%>"><%=rst1("utilitydisplay")%></a>&nbsp;
  <% 
  rst1.movenext
  loop
  rst1.movefirst
  ...
  <td style="border-top:1px solid #ffffff;" align="right">&nbsp;<%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%><input type="button" value="Add Lease Utility" onclick="leaseUtilityEdit('');" class="standard"style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"><%end if%></td>
</tr>
<tr>
...
<table width="100%" border=0 cellpadding="0" cellspacing="0">
   ...
   <tr valign="top">
    ...
    <tr>
	  ...
	  <%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%>
      <input type="button" value="Edit <%=rst1("utilitydisplay")%>" onclick="leaseUtilityEdit(<%=rst1("leaseutilityid")%>);" class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;">
	  <%end if%>
  ...
  <tr>
    ...
    <%
      ...
      if not rst2.EOF then%>
        ...
        <%do until rst2.EOF%>
        <tr bgcolor="#ffffff" onmouseover="this.style.backgroundColor = 'lightgreen'" onmouseout="this.style.backgroundColor = 'white'" onclick="meterEdit(<%=rst2("meterid")%>,<%=rst1("leaseutilityid")%>);">
          ...
          <td><span class="standard"><%=rst2("meterid")%></span></td>
        </tr>
        
        <%rst2.movenext
        loop%>
        <tr><td colspan="7" style="border-bottom:1px solid #ffffff;"><%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%><input type="button" value="Add Meter" onclick="meterAdd('',<%=rst1("leaseutilityid")%>);" id=1 name=1 class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"><%end if%></td></tr>
      <%
      else
      %>
      ...
      <tr bgcolor="#ffffff">
        <td><span class="standard" style="border-bottom:1px solid #ffffff;"><%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%><input type="button" value="Add Meter" onclick="meterAdd('',<%=rst1("leaseutilityid")%>);" id=1 name=1 class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"><%end if%></span></td>
      </tr>
      <%
      end if
      rst2.close
    ...
    <tr bgcolor="#eeeeee">
      <td style="border-top:1px solid #ffffff;border-bottom:1px solid #999999;padding:3px;"><span class="standard"><b>Lease Utilities</b></span></td>
    </tr>
    <%
    end if
    %>
  <tr>
   <td bgcolor="#dddddd" style="padding:3px;"><%if (not isBuildingOff(bldg) AND (isBuildingTransfered(pid, bldg) = 0)) then%><input type="button" value="Add Lease Utility" onclick="leaseUtilityEdit('');" class="standard" style="cursor:hand;background-color:#eeeeee;border:1px outset #ffffff;color:336699;"><%end if%>&nbsp;</td>
  </tr>
