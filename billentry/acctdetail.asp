...
{	var frm = document.forms[0];
    ...
    if(salestax < .03 && bldg != "30E60" )
    { alert("Sales tax cannot be lower than .03")
      setTimeout(function(){frm.saletaxSub.focus();}, 1);
    }
    if(salestax > 1.09)
    { alert("Sales tax cannot be higher than 1.09")
      setTimeout(function(){frm.saletaxSub.focus();}, 1);
    }
...
dim escoFixedRate ,EscoPreTax ,EscoST ,EscoGR ,EscoBillAmt ,EscoAcct, billingdatestart, billingdateend 
dim unitcostkwh ,unitcostkw ,avgCost ,totalbillamt, totalkwcost, Estimated,count, MSCDetailList, MSCDetailOffList, NoEsco
...
dim hold1OFF, hold2OFF, hold3OFF, hold4OFF, hold5OFF, hold6OFF, hold7OFF, hold8OFF, lengthOFF, diff1OFF, diff2OFF, diff3OFF, diff4OFF, diff5OFF, diff6OFF, diff7OFF, diff8OFF, kvar, rpd, gtdemand, pridemand, secdemand, gtkw
if not rst1.EOF then
	...
	billingdatestart = rst1("billingdatestart")
	billingdateend = rst1("billingdateend")
    ...
    if lengthOFF > 0 then
		...
		mscdetailOfflist = Split( MSCDetailOff, "|" )
		if IsArray(MSCDetailOffList) then
			count = ubound(MSCDetailOffList)
		end if

	    MSCSC4R1OFF = Split(MSCDetailOffList(0),"=")(1)

        MSCSC4R2OFF = Split(MSCDetailOffList(1),"=")(1)

        MSCSC9R1OFF = Split(MSCDetailOffList(2),"=")(1)

        MSCSC2R1OFF = Split(MSCDetailOffList(3),"=")(1)

	    MSCSC9RA1OFF =Split(MSCDetailOffList(4),"=")(1)

	    MSCSC9RA2OFF =Split(MSCDetailOffList(5),"=")(1)

	    MSCSC9HTR1OFF =Split(MSCDetailOffList(6),"=")(1)

	    MSCSC4RA1OFF =Split(MSCDetailOffList(7),"=")(1)

	    MSCSC4RA2OFF =Split(MSCDetailOffList(8),"=")(1)
...
'	grossrecieptSub = rst1("GrossReceipt")
  ...
  TDtotalcalc = rst1("TDtotalcalc")
	...
	kvar = rst1("kvar")
	rpd = rst1("rpd")
	gtdemand = rst1("gtdemand")
	pridemand = rst1("pridemand")
	secdemand = rst1("secdemand")
	gtkw=rst1("gtkw")
  rst1.close
  hasdata = true
%>
...
<%end if%>
    ...
    <div id="delivery" style="border: 2px solid #6699cc;height:340;position:relative;visibility:visible"> 
    <table width="100%" border="0" style="font-family:Arial, Helvetica, sans-serif; color:black">
      ...
      <tr><h1> 
		...
		Billing Start Date</td>
        <td></td>
        <td> <input type="date" name="billingdatestart" value="<%=billingdatestart%>" onKeyUp="checkvalues();">          </td>
        <td>Billing End Date</td>
        <td></td>
        <td> <input type="date" name="billingdateend" value="<%=billingdateend%>" onKeyUp="checkvalues();">          </td>        
		</h1>
      </tr>
	  <tr><td>&nbsp;</td></tr>
	  <tr>
      ...
      <tr> 
        ...
        <td>G&T Demand KW</td>
        <td></td>
        <td> <input type="tkw" name="gtkw" value="<%=gtkw%>" > 
        </td>
      <tr> 
        <td>Off Peak KWH</td>
        <td></td>
      ...
      <tr> 
        ...
        <td>No Esco<input type="checkbox" name="NoEsco" value="1" <%if trim(NoEsco)="1" then response.write " CHECKED"%>> </td>
        <td></td>
        <td></td>
        <td> 
	  ...
	  <tr>
		...
		<td>$</td>
		<td> <input type="text" name="rpd" value="<%= rpd %>"></td>
		<td>Billable KVar</td>
        <td></td>
        <td><input type="text" name="kvar" value="<%= kvar %>"></td>
		</tr>
    </table>
  </div>
<div id="commodity" style="border: 2px solid #6699cc;height:340;position:absolute;visibility:hidden">
<table width="100%" border="0" style="font-family:Arial, Helvetica, sans-serif; color:black">
<tr>
...
<div id="cap" style="border: 2px solid #6699cc;height:340;position:absolute;visibility:hidden">
...
<div id="submeter" style="border: 2px solid #6699cc;height:340;position:absolute;visibility:hidden">
<table border="0" style="font-family:Arial, Helvetica, sans-serif; color:black">
...
<tr><td width="15%">GT Demand</td><td width="1%">$/kw</td>
	<td width="24%"><input type="text" name="gtdemand" value="<%=gtdemand%>" ></td></tr>
<tr><td width="15%">Primary Demand</td><td width="1%">$/kw</td>
	<td width="24%"><input type="text" name="pridemand" value="<%=pridemand%>" ></td></tr>
<tr><td width="15%">Secondary Demand</td><td width="1%">$/kw</td>
	<td width="24%"><input type="text" name="secdemand" value="<%=secdemand%>" ></td></tr>	
</table>
...
<div id="totals" style="border: 2px solid #6699cc;height:340;position:absolute;visibility:hidden">
<table border="0" style="font-family:Arial, Helvetica, sans-serif; color:black">
