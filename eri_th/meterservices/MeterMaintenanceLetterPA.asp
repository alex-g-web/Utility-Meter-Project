...
<%end if
Response.Write "Building Meter Maintenance Letter....."
...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
  <tr>
   <form name="form1" action="MeterMaintenanceLetterPA.asp">
    <tr bgcolor="#eeeeee"> 
...
<%	
	...
	sSql = "Exec usp_MaintenanceLetterTenantsPA " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
...
' Column constants
'Const COL_FLOOR = 1
Const COL_TENANT = 1	'Const COL_TENANT = 2
'Const COL_TENANT_SQFT = 3
Const COL_TENANT_STATUS = 2	'Const COL_TENANT_STATUS = 4
'Const COL_LEASE_START = 5
'Const COL_MOVEIN_DATE = 6
'Const COL_LEASEEXP_DATE = 7
Const COL_TENANT_METER_CNT = 3	'Const COL_TENANT_METER_CNT = 8
Const COL_METER_ID = 4 	'Const COL_METER_ID = 9 
Const COL_METER_CURR_READ_STATUS = 5	'Const COL_METER_CURR_READ_STATUS = 10 
'Const COL_METER_SERIAL_NUM = 11
'Const COL_METER_MAKE = 12
'Const COL_METER_MODEL = 13
Const COL_METER_FUNCTION = 6	'Const COL_METER_FUNCTION = 14
Const COL_METER_TYPE = 7	'Const COL_METER_TYPE = 15
Const COL_METER_LOCATION_FLR = 8	'Const COL_METER_LOCATION_FLR = 16
Const COL_METER_LOCATION_ARA = 9	'Const COL_METER_LOCATION_ARA = 17
'Const COL_METER_STARTUP_DATE = 18 
Const COL_METER_READ_TYPE = 10	'Const COL_METER_READ_TYPE = 19
'Const COL_DATA_COLL_NUM = 20
'Const COL_DAT_CHANNEL_NUM = 21
Const COL_METER_READ_STATUS = 11	'Const COL_METER_READ_STATUS = 22
Const COL_METER_BILL_STATUS = 12	'Const COL_METER_BILL_STATUS = 23
Const COL_METER_USAGE = 13	'Const COL_METER_USAGE = 24
Const COL_METER_DEMAND = 14	'Const COL_METER_DEMAND = 25
...
Const COL_CUSTOMER_NOTES = 15	'Const COL_CUSTOMER_NOTES = 40
Const COL_TENANT_CHARGES = 16	'Const COL_TENANT_CHARGES = 41 
Const COL_ADMIN_FEES = 17	'Const COL_ADMIN_FEES = 42
Const COL_SALES_TX = 18	'Const COL_SALES_TX = 43
Const COL_MISC_CREDITS = 19	'Const COL_MISC_CREDITS = 44
Const COL_TENANT_TOTAL_CHGS = 20	'Const COL_TENANT_TOTAL_CHGS = 45
...
' Header Columns	
	...
	Do Until rst1.eof
		...
		Do While Not rst2.EOF 
			...
			if (lcase(rst2("MeterStatus"))="offline") Then
				'objWorkSheet.Cells(iRow,COL_METER_READ_STATUS).Font.ColorIndex = 3
				objWorkSheet.Rows(iRow).Interior.ColorIndex = 36
			end if
	...
	' Property Totals
	iRow = iRow + 1
	objWorkSheet.Cells(iRow,COL_TENANT) = "Property Totals" 
	...
	If not rst1.EOF then 
		iRow = iRow  + 1
		objWorkSheet.Cells(iRow,COL_TENANT) = "Utility Company Totals" 
	...
	' Recovery Factors
		iRow = iRow + 1
		objWorkSheet.Cells(iRow,COL_TENANT) = "Recovery Factors" 
		If not Isnull(rst1("TotalKWH")) then
		    if (Cdbl(rst1("TotalKWH")) <> 0) then 
			    objWorkSheet.Cells(iRow,COL_METER_USAGE) = (UsageTotal / Cdbl(rst1("TotalKWH"))) * 100 
			else
			    objWorkSheet.Cells(iRow,COL_METER_USAGE) = 0
			end if
		End If
		
		If not IsnUll(rst1("TotalKW")) Then
		    if (Cdbl(rst1("TotalKW")) <> 0) then 
			    objWorkSheet.Cells(iRow,COL_METER_DEMAND) = (DemandTotal /  Cdbl(rst1("TotalKW"))) * 100
			else
			    objWorkSheet.Cells(iRow,COL_METER_DEMAND) = 0
			end if
		End If
		
		If not Isnull(rst1("TotalBillAmt")) Then 
		    if (Cdbl(rst1("TotalBillAmt")) <> 0) then
			    objWorkSheet.Cells(iRow,COL_TENANT_TOTAL_CHGS) = (BuildingChargesTotal / Cdbl(rst1("TotalBillAmt"))) * 100
			else
			    objWorkSheet.Cells(iRow,COL_TENANT_TOTAL_CHGS) = 0
			end if
		End IF
	...
	'objWorkSheet.Cells(iRow,COL_TENANT) = "This maintenance report provides valuable information about your " & _
	...
	objWorkBook.SaveCopyAs("\\serv-db\web_folders\finance\PA\"  & building & "\" & building & Billyear  & Billperiod  & UtilityId & "MeterLetter.xlsx")
	...
	strFilename = "\\serv-db\web_folders\finance\PA\"  & building & "\" & building & Billyear  & Billperiod  & UtilityId & "MeterLetter.xlsx"
	If objFSO.FileExists(strFileName) Then 
	%>
	<p> Following report has been generated :
	<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="https://appserver1.genergy.com/eri_TH/finance/PA/<%=building%>/<%=building%><%=billyear%><%=billperiod%><%=UtilityId%>MeterLetter.xlsx" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=building%>MeterLetter.xlsx</b></a> 
	</p>
	<%
	Else
	%>
...
{	var frm = document.forms['form1'];
	var newhref = "MeterMaintenanceLetterPA.asp?" + "&building="+frm.building.value+"&billyear="+frm.billyear.value
	document.location.href=newhref;
}

function loadutility()
{	var frm = document.forms['form1'];
	var newhref = "MeterMaintenanceLetterPA.asp?building="+frm.building.value+"&utilityid="+frm.utilityid.value;
	document.location.href=newhref;
}
</Script>
<%
