...
' Header Columns	
	If not rst1.eof then
		...
		objWorkSheet.Cells(10,1).Font.Size = 10  
		objWorkSheet.Cells(10,1) = "Due to the NY state mandated PAUSE order, all nonessential workforce must continue to work from home." 

		objWorkSheet.Cells(11,1).Font.Bold = True
		objWorkSheet.Cells(11,1).Font.Size = 10 
		objWorkSheet.Cells(11,1) = "Our meter readers have been categorized as a nonessential employee and therefore, will be unable to" 

		objWorkSheet.Cells(12,1).Font.Bold = True
		objWorkSheet.Cells(12,1).Font.Size = 10  
		objWorkSheet.Cells(12,1) = "visit your property to obtain meter readings. Until this mandate is lifted CPL Group USA" 

		objWorkSheet.Cells(13,1).Font.Bold = True
		objWorkSheet.Cells(13,1).Font.Size = 10  
		objWorkSheet.Cells(13,1) = "will be estimating onsite meter readings, in order to continue to assist our customers in cost" 

		objWorkSheet.Cells(14,1).Font.Bold = True
		objWorkSheet.Cells(14,1).Font.Size = 10  
		objWorkSheet.Cells(14,1) = "recovery efforts." 

		objWorkSheet.Cells(16,1).Font.Bold = False
		objWorkSheet.Cells(16,1) = "" 	
		
		iRow = 17
				
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "Attn: " & rst1("contactname")
		iRow= iRow + 1
		...
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "Re: " & rst1("utilityname") & " Meter Exception Report for " & rst1("bldgname") & " Period " & Billperiod & "," & Billyear 
		iRow= iRow + 1
        ...
        objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = rst1("contactname") & ","
		iRow= iRow + 1
		...
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "For the service period referenced above, which covers " & rst1("startdate") & " through " & rst1("enddate") & ","
		iRow= iRow + 1
				
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "enclosed you will find sub meter invoices suitable for distribution as well as a sub "
		iRow= iRow + 1
		
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "meter summary report for your review."
		iRow= iRow + 1
		...
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "We kindly request your feedback on the bill period processing details listed below."
		iRow= iRow + 1
		...
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "The following meters reported no usage but were not estimated.  If you feel that based on "
		iRow= iRow + 1
				
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "your knowledge of tenant activity one or more of these meters should be estimated, please "
		iRow= iRow + 1
				
		objWorkSheet.Cells(iRow,1).Font.Bold = False
		objWorkSheet.Cells(iRow,1) = "contact our office as soon as possible:"
		iRow= iRow + 1
	...
	objWorkSheet.Cells(iRow,1).Font.Bold = False
	objWorkSheet.Cells(iRow,1) = "The following tenants reported a usage value that is either 25% higher or 25% lower when "
	...
	objWorkSheet.Cells(iRow,1).Font.Bold = False
	objWorkSheet.Cells(iRow,1) = "tenants have not been estimated.   Should you feel, based on your knowledge of tenant activity, "
	
	
	iRow= iRow + 1
	
	objWorkSheet.Cells(iRow,1).Font.Bold = False
	objWorkSheet.Cells(iRow,1) = "that one or more of these tenants should be reviewed for the purpose of estimations, please "
	...
	objWorkSheet.Cells(iRow,5) = "Floor"
	
	
	sSql = "Exec usp_MaintenanceLetter25pctUsage_bytenant " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
	rst1.CursorLocation = 3
	rst1.open sSql , cnn1, 3 
	Do Until rst1.eof
	    ...
	    objWorkSheet.Cells(iRow,5) = rst1("floordescr")
    ...
    objWorkSheet.Cells(iRow,1).Font.Bold = False
	objWorkSheet.Cells(iRow,1) = "The following tenants reported a load factor either below 20% or above 95%.  Unless management  "
	...
	objWorkSheet.Cells(iRow,1) = ""
    'objWorkSheet.Cells(iRow,3) = "Tenant"
	objWorkSheet.Cells(iRow,3) = "Floor"
    objWorkSheet.Cells(iRow,4) = "Load Factor"
	
	
	sSql = "Exec usp_MaintenanceLetterLoadFactor_bytenant " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
	...
	Do Until rst1.eof
	    ...
	    'objWorkSheet.Cells(iRow,1) = rst1("meternum")
        objWorkSheet.Cells(iRow,1) = rst1("tenant")
        objWorkSheet.Cells(iRow,3) = rst1("floordescr")
        objWorkSheet.Cells(iRow,4).HorizontalAlignment = -4131
        objWorkSheet.Cells(iRow,4) = rst1("loadfactor")
	...
	file = ucase(building) &"_"& Billyear  & "_" &Right("0" & billperiod, 2) &"_"& replace(utilname, " ", "_") & "_Meter_Letter"
	filename = file & ".xlsx"
	pdfname = file & ".pdf"
	...
	If objFSO.FileExists(strFileName) Then 
		...
		<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="http://pdfmaker.genergyonline.com/pdfMaker/<%= pdfdir %><%= filename %>?dt=<%=ctime%>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%= filename %></b></a> 
		</p>
