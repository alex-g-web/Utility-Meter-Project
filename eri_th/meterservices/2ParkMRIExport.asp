...
<%	
	...
	If billperiod <> "" then
		...
		<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="https://appserver1.genergy.com/eri_TH/finance/JLL/<%=building%>\<%=building%><%=sFileType%>_<%=MonthName(Billperiod)%><%=Billyear%>.prn" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=building%><%=sFileType%>_<%=MonthName(Billperiod)%><%=Billyear%>.prn</b></a> 
		</p>
		<%
		Else
		%>
