...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
  <tr>
   <form name="form1" action="BuildingTaxReport.asp">
    <tr bgcolor="#eeeeee"> 
      <td colspan="2" style="border-top:1px solid #ffffff;border-bottom:1px solid #cccccc;"> 
        <table border=0 cellpadding="3" cellspacing="0">
          <tr> 
				...
				<td> <select name="utilityid" onChange="document.location='BuildingTaxReport.asp?bldgnum=<%=building%>&utilityid='+this.value">
            ...
            <td> <select name="billyear" onChange="document.location='BuildingTaxReport.asp?bldgnum=<%=building%>&utilityid=<%=utilityid%>&billyear='+this.value">
...
' Header Columns	
	If not rst1.eof then
        ...
        objWorkSheet.Cells(8,14).Font.Bold = False
		objWorkSheet.Cells(8,14) = "RXR G/L Accounts (Revenue accts & Sales Tax Payable Acct)" 
						
		objWorkSheet.Cells(9,1).Font.Bold = False
		objWorkSheet.Cells(9,14) = "4061"
        objWorkSheet.Cells(9,15) = "4406"
        objWorkSheet.Cells(9,16) = "2025"
        objWorkSheet.Cells(9,17) = "2025"
        objWorkSheet.Cells(9,14).Interior.ColorIndex = 44
        objWorkSheet.Cells(9,15).Interior.ColorIndex = 44 
        objWorkSheet.Cells(9,16).Interior.ColorIndex = 40
        objWorkSheet.Cells(9,17).Interior.ColorIndex = 40
        objWorkSheet.Cells(9,18) = ""
        objWorkSheet.Cells(9,19) = "Sales Tax Filing"	
        objWorkSheet.Cells(9,18).Interior.ColorIndex = 43
        objWorkSheet.Cells(9,19).Interior.ColorIndex = 43	
        ...
        objWorkSheet.Cells(10,9) = "Billed Consumption (Pre-Tax)" 
        objWorkSheet.Cells(10,10) = "Tax on Cost"
        objWorkSheet.Cells(10,11) = "Tax on Admin Fee"
        objWorkSheet.Cells(10,12) = "Total with Tax"
        objWorkSheet.Cells(10,13) = "Load Code" 
        objWorkSheet.Cells(10,14) = "RXR Sub Meter Income (EM)"
        objWorkSheet.Cells(10,15) = "RXR Admin Fee Income (AS)"
        objWorkSheet.Cells(10,16) = "RXR Salex Tax Electric (SE)"
        objWorkSheet.Cells(10,17) = "RXR Remit Tax on Admin Fee (SY)" 
        objWorkSheet.Cells(10,18) = "Tax to be remitted"
        objWorkSheet.Cells(10,19) = "Tax not to be remitted - Recorded as JE against GL#5240 - Electric expense."
        objWorkSheet.Cells(10,20) = "Check Calc."
        objWorkSheet.Cells(10,14).Interior.ColorIndex = 44
        objWorkSheet.Cells(10,15).Interior.ColorIndex = 44 
        objWorkSheet.Cells(10,16).Interior.ColorIndex = 40
        objWorkSheet.Cells(10,17).Interior.ColorIndex = 40
        objWorkSheet.Cells(10,18).Interior.ColorIndex = 43
        objWorkSheet.Cells(10,19).Interior.ColorIndex = 43	
	...
	iRow = 10
	sSql = "Exec usp_BuildingTaxReport " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
	rst1.CursorLocation = 3
	rst1.open sSql , cnn1, 3 
	Do Until rst1.eof
        ...
        objWorkSheet.Cells(iRow,9) = rst1("billconsumption")
        objWorkSheet.Cells(iRow,10) = rst1("taxoncost")
        objWorkSheet.Cells(iRow,11) = rst1("taxonadminfee")
        objWorkSheet.Cells(iRow,12) = rst1("totalwithtax")
        objWorkSheet.Cells(iRow,13) = rst1("loadcode")
        objWorkSheet.Cells(iRow,14) = rst1("RXRSubMeterIncomeEM")
        objWorkSheet.Cells(iRow,15) = rst1("RXRAdminFeeIncomeAS")
        objWorkSheet.Cells(iRow,16) = rst1("RXRSalexTaxElectricSE")
        objWorkSheet.Cells(iRow,17) = rst1("RXRRemitTaxonAdminFeeSY")
        objWorkSheet.Cells(iRow,18) = rst1("Taxtoberemitted")
        objWorkSheet.Cells(iRow,19) = rst1("Taxnottoberemitted")
        objWorkSheet.Cells(iRow,20) = rst1("CheckCalc")
	    objWorkSheet.Cells(iRow,14).Interior.ColorIndex = 44 
        objWorkSheet.Cells(iRow,15).Interior.ColorIndex = 44 
        objWorkSheet.Cells(iRow,16).Interior.ColorIndex = 40 
        objWorkSheet.Cells(iRow,17).Interior.ColorIndex = 40 
        objWorkSheet.Cells(iRow,18).Interior.ColorIndex = 43 
        objWorkSheet.Cells(iRow,19).Interior.ColorIndex = 43 
						
		rst1.movenext
	loop
	rst1.close

    objWorkSheet.Cells(iRow,19).Font.ColorIndex = 3
	
	iRow= iRow + 1

    sSql = "Exec usp_BuildingTaxReportTotals " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
	rst1.CursorLocation = 3
	rst1.open sSql , cnn1, 3 
	Do Until rst1.eof
        ...
        if rst1("lineorder") = 1 then
           objWorkSheet.Cells(iRow,12).Font.ColorIndex = 8
        end if
        if rst1("lineorder") = 8 then
           objWorkSheet.Cells(iRow,1).Font.ColorIndex = 7
        end if
        if rst1("lineorder") = 3 then
           objWorkSheet.Cells(iRow,17).Font.ColorIndex = 7
           objWorkSheet.Cells(iRow,19).Font.ColorIndex = 10
        end if
    ...
    sSql = "Exec usp_BuildingTaxReportTotals2 " & "'" & building & "'" & ", " & UtilityId  & "," & Billyear & "," & BillPeriod 
	rst1.CursorLocation = 3
	rst1.open sSql , cnn1, 3 
	Do Until rst1.eof
	...
	objWorkBook.SaveCopyAs("\\serv-db\web_folders\finance\"  & building & Billyear  & Billperiod  & UtilityId & "BuildingTaxReport.xlsx")
	...
	strFilename = "\\serv-db\web_folders\finance\" & building & Billyear  & Billperiod  & UtilityId & "BuildingTaxReport.xlsx"
	If objFSO.FileExists(strFileName) Then 
	%>
	<p> Following report has been generated :
	<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="https://appserver1.genergy.com/eri_TH/finance/<%=building%><%=billyear%><%=billperiod%><%=UtilityId%>BuildingTaxReport.xlsx" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=building%>BuildingTaxReport.xlsx</b></a> 
	</p>
	<%
	Else
	%>
