...
<%
	...
	for i = 1 to maxPageCount
			strFileName	= "C:\ghnet_websites\genergyonline.com\pdfMaker\" & portfolio & "\" & building & "\" & building & byear & bperiod & utilityid & i & ".pdf"		
			If objFSO.FileExists(strFileName) Then 
				blnBillsAvailable = True
			%>
			<tr><td Colspan=2>
			<a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%=portfolio%>/<%=building%>/<%=building%><%=byear%><%=bperiod%><%=utilityid%><%=i%>.pdf" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'"><b><%=i%> page Bills pdf: <%=building%><%=byear%><%=bperiod%><%=utilityid%><%=i%>.pdf</b></a> 
			</td></tr>
			<%
			Else %>
				<tr><td Colspan=2>
