...
function getConnect(pid, bldg, class_req)
	...
	Select case lcase(trim(class_req))
		case "billing" 
			CLASSSTRING="dbBilling;ConnectTimeout=0;"
			PIDSTR = "PID" & PIDVAL
		case "intervaldata" 
		    CLASSSTRING="dbIntervalData;ConnectTimeout=0;"
			PIDSTR = "PID" & PIDVAL
		case "archive" 
			CLASSSTRING="dbArchiveIntervalData;ConnectTimeout=0;"
			PIDSTR = "PID" & PIDVAL
		case "dbcore" 
			CLASSSTRING="dbCore;ConnectTimeout=0;"
			PIDSTR = "dbDefault"
		case "intranet" 
			CLASSSTRING="main;ConnectTimeout=0;"
			PIDSTR = "dbIntranet"
		case "engineering" 
			CLASSSTRING="dbEngineering;ConnectTimeout=0;"
			PIDSTR = "dbDefault"
		case else 
...
Sub Checkxml()
  if isempty(session("xmlUserObj")) then
    response.write "There is no user session established, please login again.</br>"
	'dim name
