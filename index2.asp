...
if uname <> "" then 
	...
	if logincomment = "1" then 
		response.redirect "https://appserver1.genergy.com/genergy2/index.htm"
	%>
	<script>
	
	var temp = "https://<%=request.servervariables("server_name")%>/genergy2/index.htm"
	...
	if message = "" then message="UNKNOWN SYSTEM ERROR<br>PLEASE TRY AGAIN LATER" end if
	end if 
end if
%>
<html>
...
<% response.write "ADDR:"&request.servervariables("local_addr") %>
<form method="POST" action="index2.asp" name="FrontPage_Form1">
  ...
  <tr> 
    ...
    <table border=0 cellpadding="3" cellspacing="0" width="228">
                ...
                <tr> 
                  ...
                  <td><input type="text" name="uname" id="unameId" size="16" class="standard" value="<%=uname%>"></td>
                </tr>
                <tr> 
                  <td align="right"><span class="standard">Password</span></td>
                  <td><input type="password" name="pword" id="pwordId" size="16" class="standard"> 
                    <input name="org" type="hidden" id="group" value="genergy"> 
                  </td>
                </tr>
                <tr>
                  <td colspan="2" align="center"> <input type="image" src="images/login.gif" name="btnLogin" id="btnLoginId" value="Login" class="standard" onMouseOver="msover(this);" onMouseOut="msout(this);"> 
