...
function viewH2OUsage(bldgnum, meternum, byear, bperiod)
{
		cWin = window.open("viewH2OUsage.asp?bldgnum="+bldgnum+"&meternum="+meternum+"&byear="+byear+"&bperiod="+bperiod,"H2O Daily Usage","width=650,height=900,resizable=yes,scrollbars=yes");
		cWin.focus();
}
      ...
      <table border=0 cellpadding="3" cellspacing="0">
        <tr valign="bottom"> 
          ...
          <td> <select name="building" onChange="loadbuilding()">
              ...
              <%
								rst1.open "SELECT BldgNum, strt FROM buildings b WHERE portfolioid='"&pid&"' ORDER BY offline asc, strt", getConnect(pid,building,"billing")
								if rst1.eof then
									rst1.close
									rst1.open "SELECT BldgNum, strt FROM buildings b WHERE portfolioid='"&pid&"' ORDER BY offline asc, strt", getConnect(pid,building,"dbCore")
								end if
								do until rst1.eof%>
					...
					<td>
						...
						<%if not(isBuildingOff(building)) then%>
							...
							<% if hasInterval = 1 then %>
								<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="viewH2OUsage('<%= building %>','<%= Wmeternum %>', <%= year(thisdate) %>, <%= month(thisdate) %>);" class="mgmtlink">View H2O Usage</a>
							<% end if %>
						<%end if%>
					</td>
				</tr>
