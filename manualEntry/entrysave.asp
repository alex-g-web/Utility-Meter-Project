...
if isnumeric(request("rawusedoff")) and trim(request("rawusedoff"))<>"" then rawusedoff = request("rawusedoff") else rawusedoff = 0
if isnumeric(request("rawusedint")) and trim(request("rawusedint"))<>"" then rawusedint = request("rawusedint") else rawusedint = 0
...
if isnumeric(request("coincd")) and trim(request("coincd"))<>"" then coincd = request("coincd") else coincd = 0

dim cnn1, rst1, strsqlc, strsqlp, strsqlcoincd
...
if trim(action)="Save" then
	if trim(rawcurrent)="" then rawcurrent=0
		...
		strsqlp = "INSERT INTO peakdemand (meterid, billyear, billperiod, datepeak, estimated, usernote, rawdemand, rawprev, datepeak_off, datepeak_int, rawdemand_off, rawdemand_int, rawprev_off, rawprev_int, coincd) values ("&meterid&", "&wyear&", "&wperiod&", '"&datepeak&"', '"&estimatedp&"', '"&usernotep&"', '"&rawdemand&"', '"&rawprev&"', '"&datepeak_off&"', '"&datepeak_int&"', '"&rawdemand_off&"', '"&rawdemand_int&"', '"&rawprev_off&"', '"&rawprev_int&"', '"&coincd&"')"
	...
	elseif trim(action)="Update" then
		...
		strsqlp = "UPDATE peakdemand set datepeak='"&datepeak&"', estimated='"&estimatedp&"', usernote='"&usernotep&"', demand="&rawdemand&", rawdemand="&rawdemand&", rawprev='"&rawprev&"', datepeak_off='"&datepeak_off&"', datepeak_int='"&datepeak_int&"', rawdemand_off='"&rawdemand_off&"', rawdemand_int='"&rawdemand_int&"', rawprev_off='"&rawprev_off&"', rawprev_int='"&rawprev_int&"' WHERE meterid="&meterid&" and billyear="&wyear&" and billperiod="&wperiod
...
strsqlcoincd = "update  peakdemand set coincd="&coincd&" where billyear="&wyear&" and billperiod="&wperiod&" and meterid in (select meterid from meters where LeaseUtilityId=(select leaseutilityid from meters where meterid ="&meterid&"))"
cnn1.Execute strsqlcoincd
'response.end()
