...
dim byear, bperiod, meterid, tnum, tname, building, isposted, pid, utilityid, yscroll,viewtype, status, rst5, sql, thisdate
...
'	if not(isnumeric(UsedInt)) or Trim(UsedInt) = ""  then UsedInt = 0
	...
	cmd.Parameters.Append prm
    Set prm = cmd.CreateParameter("charge", adInteger, adParamInput)
	cmd.Parameters.Append prm
	'set connection
	...
	<script>
		...
		opener.document.location.href = "re_index.asp?t=<%=viewtype%>&pid=<%=pid%>&building=<%=building%>&byear=<%=byear%>&utilityid=<%=utilityid%>&bperiod=<%=bperiod%>&yscroll="+yscroll+"&yscroll2="+yscroll2+"&showscroll="+showscroll+"&showscroll2="+showscroll2+"&mscroll="+mscroll;
		window.close();
	</script>
	<%response.end
end if
...
{	var frm = document.forms['form1']
	...
	frm.submit();
}https://appserver1.genergy.com/genergy2/validation/update_billentry.asp?t=m&tid=52034&meterid=68136&byear=2024&bperiod=7&tname=[Panera++]&tnumber=TN52034&building=110UP&pid=73&utilityid=3&posted=False#

function viewH2OUsage(bldgnum, meternum, byear, bperiod, utilityid)
{
		cWin = window.open("/genergy2/manualentry/viewH2OUsage.asp?bldgnum="+bldgnum+"&meternum="+meternum+"&byear="+byear+"&bperiod="+bperiod+"&utilityid="+utilityid,"H2O Daily Usage","width=650,height=900,resizable=yes,scrollbars=yes");
		cWin.focus();
}
...
<table border=0 cellpadding="3" cellspacing="0" width="100%">
	<tr>
		...
		<td bgcolor="#6699cc" align="right">&nbsp;&nbsp;
			...
			<a class="standardheader" href="#" onClick="<%=meterSetupLink%>">Meter Setup</a>&nbsp;<%
			...
			if trim(datasource) = ""  OR ISNULL(datasource) then		
				...
				<a class="standardheader" href="#" onClick="<%=LMPURL%>">Meter LMP</a>&nbsp;				
				<a class="standardheader" href="<%=pulseURL%>" target="_blank">View Interval Data</a><%
			end if%>
		</td>
	</tr>
</table>
...
<table width="100%" border="0" cellspacing="0" cellpadding="3">
	<tr bgcolor="#dddddd">
        ...
        <td class="bordercell">Charge</td>
		<td class="bordercell">&nbsp;</td>
		<td class="bordercell">Consumption Notes</td>
		<td class="bordercell">Demand Notes</td>
		<td class="bordercell">Meter Letter Customer Notes</td>
		<td class="bordercell">Meter Letter Internal Notes</td>
	</tr>
	<%
	dim currentbp
	if not rst1.EOF then
		...
		<tr valign="top" bgcolor="#eeeeee">
			...
			<%if isposted<>"True" and not(isbuildingOff(building)) then%>
                ...
                <td><input type="checkbox" name="charge" value="1" <%if rst1("charge")="True" then response.write "CHECKED"%>></td>
				<%origLink(1)%>
				<td><textarea cols="15" rows="" name="note"><%=rst1("note")%></textarea></td>
				<td><textarea cols="15" rows="" name="pdnote"><%=rst1("pdnote")%></textarea></td>
				<td><textarea cols="15" rows="" name="mlcnote"><%=rst1("MLCustomernotes")%></textarea></td>
				<td><textarea cols="15" rows="" name="mlinote"><%=rst1("MLInternalnotes")%></textarea></td>				
				</tr>
				<tr bgcolor="#eeeeee">
	...
	do until rst1.EOF
		if trim(rst1("billperiod"))<>currentbp then
			...
			<tr valign="top" bgcolor="#ffffff">
                ...
                <td class="bordercolumn"><%=rst1("charge")%>&nbsp;</td>
				<%origLink(0)%>
...
%>
<%sub origLink(t)%>
	<td><a href="#" onClick="open('original_meterreadings.asp?utilid=<%=utilityid%>&meterid=<%=meterid%>&bperiod=<%=rst1("BillPeriod")%>&byear=<%=rst1("BillYear")%>&bldgnum=<%=building%>','','width=210,height=230')">see&nbsp;orig.&nbsp;read</a></br>
	<% if hasInterval = 1  then %>
		<%
			'thisdate = dateadd("m", -1, now())
			thisdate = dateserial(rst1("BillYear"),rst1("BillPeriod"),1)
		%>
		<a href="#" onClick="viewH2OUsage('<%= building %>','<%= rst1("meternum") %>', <%= year(thisdate) %>, <%= month(thisdate) %>, <%= utilityid %>);" class="mgmtlink">View H2O Data</a>
	<% end if %></td>
