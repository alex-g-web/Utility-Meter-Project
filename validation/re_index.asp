...
dim rst1, cnn1, strsql, cnn2, cnn3,rst4
...
<form name="form1" method="get" action="">	
  <table border=0 cellpadding="3" cellspacing="0" width="100%">
	...
	document.location='re_index.asp?pid=<%=pid%>&building=<%=building%>&utilityid=<%=utilityid%>&bperiod=<%=request("bperiod")%>&historic=<%if historic then%>false<%else%>true<%end if%>'
...
<%if trim(bperiod)<>0 then
	...
	dim super, btabcolor, stabcolor, flagColor, flagColorHilight, procPage, prevbyear, prevbperiod, str, rst2
	set rst2 = server.createobject("ADODB.Recordset")
    
	
	//3/24/2008 N.Ambo placed this section here (it was written below)
	prevbyear = byear
	prevbperiod = bperiod-1		
		
	if prevbperiod<1 then 		
		prevbyear = prevbyear-1
		//3/21/2008 N.Ambo added if statement to differnetiate between the maxperiod for 12 period and 4 period years	
		//4/22/2008 N.Ambo amended sql statement to default billperiod to 0 if no value was found
		str = "select isnull(max(billperiod),0) as billperiod from billyrperiod where bldgnum ='"&building&"' and billyear='"&prevbyear&"' and utility = "&utilityid
		rst2.Open str, cnn3
		if not rst2.EOF then
			prevbperiod = rst2("billperiod")
	...
	dim mainSql
		...
		procPage = "super_process.asp?t="&t&""
		pagelink = "s"
		if Cint(pid) = 7 and cint(utilityid)= 6 then
			mainSql = _
				...
				"LEFT JOIN Custom_OucBill CB ON c.billyear=CB.billyear " & _
				" AND c.billperiod=CB.billperiod AND m.LeaseutilityId = CB.LeaseUtilityId  " & _
				"LEFT JOIN ("&_
	    ...
	    //N.Ambo 3/24/2008 added this piece for JFK which has 4 periods for cold water
	    elseif building = "JFK" and cint(utilityid)= 3 then
			mainSql = _
				"SELECT (case when (avgKWH=0 or avgKW=0 or kwhvarience > variance*100+8 or kwvarience > variance*100 or ((kwhOFFvarience > variance*100+8 or kwhINTvarience > variance*100+8 or kwOFFvarience > variance*100 or kwINTvarience > variance*100) and extusg=1) or AMTvarience > variance*100) then '0' else '1' end) as belowVarience, * FROM ("&vbcrlf&_
				...
				"INNER JOIN tblleases l on lup.billingid=l.billingid "&_
				"LEFT JOIN ("&_
				"SELECT c2.meterid, isNull(avg(used),0) as avgKWH, avg(usedoff) as avgKWHoff, avg(usedint) as avgKWHint, isNull(avg(demand),0) as avgKW, avg(demand_off) as avgKWoff, avg(demand_int) as avgKWint FROM consumption c2 INNER JOIN peakdemand d2 ON d2.meterid=c2.meterid and c2.billyear=d2.billyear and c2.billperiod=d2.billperiod WHERE (d2.billyear="&prevbyear&" and d2.billperiod="&prevbperiod&") GROUP BY c2.meterid "&_
				") CAvg ON CAvg.meterid=m.meterid "&_
				"LEFT JOIN ("&_
				"SELECT leaseutilityid, isNull(avg(totalamt),0) as avgAmt FROM tblbillbyperiod WHERE (billyear="&prevbyear&" and billperiod="&prevbperiod&") and reject=0 GROUP BY leaseutilityid "&_
				") BBAvg ON BBAvg.leaseutilityid=lup.leaseutilityid "&_
				"LEFT JOIN coincidentdemand cd on cd.leaseutilityid = lup.leaseutilityid and cd.billyear = c.billyear and cd.billperiod = c.billperiod "&_
				"LEFT JOIN tblbillbyperiod bbp on bbp.reject=0 and m.leaseutilityid=bbp.leaseutilityid and c.billyear=bbp.billyear and c.billperiod=bbp.billperiod "&_
				"LEFT JOIN validation v on m.Meterid=v.Meterid and c.billyear=v.billyear and c.billperiod=v.billperiod "&_
			...
			mainSql = _
				...
				mainSql = "Exec usp_ReviewEditMeters '" & building & "'," & byear & ", " & bperiod & "," & utilityid
			end if
	...
	numoftenantsPrev =0
	numofmetersPrev =0
	dim  strsql2 'rst2, prevbyear, prevbperiod, str
	'set rst2 = server.createobject("ADODB.Recordset")
	'prevbyear = byear
	'prevbperiod = bperiod-1
	'if prevbperiod<1 then 		
	'	prevbyear = prevbyear-1
	'	
	'	str = "select max(billperiod) as billperiod from billyrperiod where bldgnum ='"&building&"' and billyear='"&prevbyear&"' and utility = "&utilityid
	'	rst2.Open str, cnn3
	'	if not rst2.EOF then
	'		prevbperiod = rst2(billperiod)
	'	end if
	...
	rst2.close	
	checksql = "sp_check_consumption_details  '" & building & "'," & byear & ", " & bperiod & "," & prevbyear & ", " & prevbperiod & "," & utilityid
            ...
            &nbsp;&nbsp;Period&nbsp;<%=bperiod%><%=displaydate%>,&nbsp;<%=byear%></span>  <b>[<a href="#" onClick="window.open('/genergy2_intranet/itservices/ttracker/troublesearch.asp?searchstring=<%=building%>&action=Search&searchbox=false&buildings=True','SearchNotes','width=800,height=400, scrollbars=no')">there are <%=tcount%> criticl tickets open for this building</a>]</b>
				...
				<table border=0 cellpadding="3" cellspacing="1" width="100%">
					...
					<tr>
						...
						<td align="right" colspan="3" bgcolor="#eeeeee">&nbsp;</td>
				  </tr>
				</table>
                <%
                rst4.open checksql, cnn1
                ...
                <table border=0 cellpadding="3" cellspacing="0">
                    ...
                    <tr>
						<td width="8%">Manual Entry Meter Issues</td>
					</tr>
				</table>
                <table border=0 cellpadding="3" cellspacing="1" width="100%">
					<tr bgcolor="#dddddd">
						<td width="10%" align="left">Meter</td>
						<td width="10%" align="left">Previous On Matches</td>
						<td width="12%" align="left">Previous Off Matches</td>
						<td width="10%" align="left">Pervious Int Matches</td>
					</tr>
                    <% do until rst4.eof 
			        %>
                       <tr>
                          <td> <span class="standard"><%=rst4("meternum")%></span></td>
			              <td> <span class="standard"><%=rst4("prevonmatch")%></span></td>
                          <td> <span class="standard"><%=rst4("prevoffmatch")%></span></td>
			              <td><span class="standard"><%=rst4("previntmatch")%></span></td>
                       </tr>
                    <% rst4.movenext
		            loop %>
                </table>
				...
				<table border=0 cellpadding="3" cellspacing="0">
					<tr>
						<td>
						View <input type="radio" name="t" value="t" onClick="switchView(this);" <%if t="t" then response.Write "checked"%>> Tenant <input type="radio" name="t" value="m" <%if t="m" then response.Write "checked"%> onClick="switchView(this);"> Meter 
						</td>
					</tr>
				</table>
...
<%
	...
	%>
		...
		<%if not(isBuildingOff(building)) then%>
			<table border=0 cellpadding="3" cellspacing="0" width="100%">
				<tr>
					<td style="padding-top:4px;" bgcolor="#eeeeee" width="30">
...
function listmeters(tag, displaymode, rmode)
	...
	Dim cTenant,endLoop,tagcolor_u,tagcolor_d,ischecked , byear2
	...
	%>
		...
		<tr><td>&nbsp;</td><td align="left"><span id="d_<%=tag%>_<%=rst1("billingid")%>_row" name="tenantlist_titles_<%=tag%>">[+]</span>&nbsp;<a href="javascript:display('<%=tag%>_<%=rst1("billingid")%>_row')"><%=rst1("billingname")%></a> [ TN: <%=rst1("tenantnum")%> BID: <%=rst1("billingid")%> LID: <%= rst1("leaseutilityid") %> ]</td></tr>
...
function listOriginalmeters(tag, displaymode, rmode)
	if not isnumeric(rmode) then rmode=0%>
	<div id="<%=tag%>" style="display:<%=displaymode%>;overflow:scroll;">
		<table width="100%" border=0 cellspacing="1" cellpadding="3">
			<tr bgcolor="#dddddd" valign="bottom">
			<%if (Cint(pid) = 7 and  cint(utilityid)= 6) or (building="JFK" and cint(utilityid)= 3)  then %>
			<td width="65"><font color="black">Accept</font></td>
			<td width="<%=cellw+20%>"><span class="standardheader"><font color="black">Tenant&nbsp;No</font></span></td>
			<td width="<%=cellw+15%>"><span class="standardheader"><font color="black">Tenant&nbsp;Name</font></span></td>
			<td width="<%=cellw%>"><span class="standardheader"><font color="black">Tenant W/Sqft.</font></span></td>
			<td width="<%=cellw+15%>"><span class="standardheader"><font color="black">Meter&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font></span></td>
			...
			<td width="<%=cellw%>"><span class="standardheader"><font color="black">Contracted <%=demand%></font></span></td>
			<td width="<%=cellw%>"><span class="standardheader"><font color="black">Billed <%=demand%></font></span></td>
			
			<td width="<%=cellw%>"><span class="standardheader"><font color="black">Bill Amount</font></span></td>
			...
			<%Else%>
			<td width="5%"><font color="black">Accept</font></td>
			<td width="5%"><span class="standardheader"><font color="black">Tenant&nbsp;No</font></span></td>
			<td width="8%"><span class="standardheader"><font color="black">Tenant&nbsp;Name</font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Tenant W/Sqft.</font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Meter&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font></span></td>
			
			<td width="4%"><span class="standardheader"><font color="black">Load Factor</font></span></td>
			<td width="4%"><span class="standardheader"><font color="black">Current <%=usage%> Usage</font></span></td>
			<td width="4%"><span class="standardheader"><font color="black">Variance <%=usage%></font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Last BP <%=usage%> Variance</font></span></td>	
			<td width="5%"><span class="standardheader"><font color="black">Last Yr BP <%=usage%> Variance</font></span></td>	
			
			
			<td width="5%"><span class="standardheader"><font color="black">Current <%=demand%></font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Variance <%=demand%></font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Last BP <%=demand%> Variance</font></span></td>	
			<td width="5%"><span class="standardheader"><font color="black">Last Yr BP <%=demand%> Variance</font></span></td>	
			
			
			<td width="5%"><span class="standardheader"><font color="black">Bill Amount</font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Average Amount</font></span></td>
			<td width="5%"><span class="standardheader"><font color="black">Variance Amount</font></span></td>
			<td width="10%"><span class="standardheader"><font color="black">Updated <%=usage%>/<%=demand%></font></span></td>
			<%End If %>			
		
			</tr>
		</table>
		<div id="meterlist" style="overflow:auto;height:auto;border: 1px solid #cccccc;margin:3px;width:100%;">
			<table width="100%" border="0" cellspacing="1" cellpadding="3">
				<%
				reccount=0
				previousMeterid = ""
				do until rst1.eof 
					...
					<%if (Cint(pid) = 7 and  cint(utilityid)= 6) or (building="JFK" and cint(utilityid)= 3)  then %>
						<td width="70">
						<%if not isposted then%><nobr>&nbsp;
							<input type="checkbox" value="<%=rst1("meterid")%>" name="meters" onMouseOver="checkboxf=1" onMouseOut="checkboxf=0" style="cursor:auto" <%=ischecked%>></nobr>
						<%else%>
							<nobr>S. Acc</nobr>
						<%end if%>
						</td>
					
						<td width="<%=cellw%>"><nobr>&nbsp;&nbsp;&nbsp;<%=trim(rst1("tenantnum"))%></nobr></td>
						<td width="<%=cellw+15%>"><nobr><%=tenantname%></nobr></td>
						<td width="<%=cellw%>"><nobr><% 
							If not Isnull(rst1("wsqft")) then 
								Response.Write formatnumber(rst1("wsqft"),3)
							Else
								Response.Write "------ "
							End If
							%>&nbsp;&nbsp;</nobr></td>
						<td width="<%=cellw+15%>"><%=trim(rst1("meternum"))%></td>
						...
						<td width="<%=cellw%>"><b><%if extusg then response.write "On:"%><%=formatnumber(kwhvartemp,0)%>%
						...
						<td width="<%=cellw%>"><nobr>&nbsp;&nbsp;&nbsp;<%=formatcurrency(rst1("totalamt"),2)%></nobr></td>
						<td width="<%=cellw%>">&nbsp;&nbsp;&nbsp;<%=formatcurrency(rst1("avgAmt"),2)%></td>
						<td width="<%=cellw%>"><b><nobr>&nbsp;&nbsp;&nbsp;<%=dollvarbold%><%=formatnumber(rst1("Amtvarience"),0)%>%&nbsp;&nbsp;&nbsp;</nobr></b></td>
							<td width="<%=cellw%>">
					...
					<% Else %>
						...
						<td width="5%"><nobr>&nbsp;&nbsp;&nbsp;<%=trim(rst1("tenantnum"))%>&nbsp;&nbsp;</nobr></td>
						<td width="8%"><nobr>&nbsp;&nbsp;&nbsp;<%=tenantname%></nobr></td>
						<td width="5%"><nobr>&nbsp;&nbsp;&nbsp;<% 
							If not Isnull(rst1("wsqft")) then 
								Response.Write formatnumber(rst1("wsqft"),3)
							Else
							...
							End If
							%>&nbsp;&nbsp;</nobr></td>
						<td width="5%"><nobr>&nbsp;&nbsp;<%=trim(rst1("meternum"))%></nobr></td>
						
						<%if Cdbl(rst1("LoadFactor")) > 1 then %>
							<td width="4%"><font color=red><nobr>&nbsp;&nbsp;<%=trim(rst1("LoadFactor"))%></nobr></font></td>
						<%else%>
							<td width="4%"><nobr>&nbsp;&nbsp;<%=trim(rst1("LoadFactor"))%></nobr></td>
						<%end if%>
						
                        <%if Cdbl(rst1("kwhused")) < 0 then %>
						  <td width="4%"><font color=red>&nbsp;&nbsp;&nbsp;<%if extusg then response.write "On:"%><%=formatnumber(rst1("kwhused"),0)%>
                        <%else%>
                           <td width="4%">&nbsp;&nbsp;&nbsp;<%if extusg then response.write "On:"%><%=formatnumber(rst1("kwhused"),0)%>
                        <%end if%>

						<%if extusg then%><br><nobr>Off:<%=formatnumber(rst1("KWHoff"),0)%></nobr><br><nobr>Int:<%=formatnumber(rst1("KWHint"),0)%></nobr><%end if%>
						</td>

						<td width="4%"><b>&nbsp;&nbsp;&nbsp;<%if extusg then response.write "On:"%><%=formatnumber(kwhvartemp,0)%>%
						<%if extusg then%><br><nobr>Off:<%=formatnumber(rst1("KWHoffvarience"),0)%>%</nobr><br><nobr>Int:<%=formatnumber(rst1("KWHintvarience"),0)%>%</nobr></b><%end if%>
						</td>
						<td width="5%"><b><nobr>&nbsp;&nbsp;&nbsp;<% 
							If not Isnull(rst1("LastBPKWHVar")) then 
								Response.Write formatnumber(rst1("LastBPKWHVar"),2)
							Else 
								Response.Write "-"
							End If
						%>%</nobr></b></td>
						<td width="5%"><b><nobr>&nbsp;&nbsp;&nbsp;<% 
							If not Isnull(rst1("LastYRKWHVar")) then 
								Response.Write formatnumber(rst1("LastYRKWHVar"),2)
							Else 
								Response.Write "-"
							End If
						%>%</nobr></b></td>
						<td width="5%"><nobr>&nbsp;&nbsp;
						<%if extusg then response.write "On:"%>
							<%if cdbl(rst1("demand"))<0 then %> <font color=red> <%=formatnumber(rst1("demand"),2)%> </font>
							<% else %>
								<%=formatnumber(rst1("demand"),2)%> 
							<% end if %>
						<%if lcase(trim(rst1("coincident"))) = "true" or lcase(trim(rst1("coincident_peak"))) = "true"then%>
						<b><%=formatnumber(rst1("coindemand"),2)%></b>
						<%end if%>
						...
						<td width="5%"><%=kwhvarbold%>&nbsp;&nbsp;<%if extusg then response.write "On:"%><%=formatnumber(kwvartemp,0)%>%
						<%if extusg then%><br><nobr>Off:<%=formatnumber(rst1("KWoffvarience"),0)%>%</nobr><br><nobr>Int:<%=formatnumber(rst1("KWintvarience"),0)%>%</nobr><%end if%>
						</td>
						<td width="5%"><b><nobr>&nbsp;&nbsp;&nbsp;<% 
							If not Isnull(rst1("LastBPKWVar")) then 
								Response.Write formatnumber(rst1("LastBPKWVar"),2)
							Else 
								Response.Write "-"
							End If
						%>%</nobr></b></td>
						...
						<td width="5%"><b><nobr>&nbsp;&nbsp;&nbsp;<% 
							If not Isnull(rst1("LastYRKWVar")) then 
								Response.Write formatnumber(rst1("LastYRKWVar"),2)
							Else 
								Response.Write "-"
							End If
						%>%</nobr></b></td>
						<td width="5%"><nobr><%=formatcurrency(rst1("totalamt"),2)%></nobr></td>
						<td width="5%"><%=formatcurrency(rst1("avgAmt"),2)%></td>
						<td width="5%"><b><nobr><%=dollvarbold%><%=formatnumber(rst1("Amtvarience"),0)%>%&nbsp;&nbsp;&nbsp;</nobr></b></td>
							<td width="10%">
							<%if rst1("validate")="True" then%>
								Accepted<br>
							<%end if%>
