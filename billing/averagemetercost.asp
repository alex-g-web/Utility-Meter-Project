...
<%end if
	...
	rst0.open "select bldgnum from DailyExportBuildings where online=1 and pid = "&portfolioid&" and utilityid = "&utilityid&" order by bldgnum asc", cnn1
	...
	do until rst0.eof 
		...
		if not rst1.eof then
			...
			case else
				...
				csvFile = ucase(building) & "_" & csvPeriod & "_" & csvUtility & "_" & "Average_Meter_Cost_Report" & ".txt"
				PcsvFile = "Residential" & "_" & csvPeriod & "_" & csvUtility & "_" & "Average_Meter_Cost_Report" & ".csv"
		...
		Do Until rst1.eof
			...
			do until rst2.eof
			   ...
			   'response.end
				...
				csvTenant(tMeter).building = ucase(building)   										'building
				csvTenant(tMeter).tNum = ucase(rst1("TenantNum"))   						't num
				csvTenant(tMeter).tName = rst1("billingname") 						't name
				csvTenant(tMeter).mNum = ucase(rst2("Meternum")) 							'meter
