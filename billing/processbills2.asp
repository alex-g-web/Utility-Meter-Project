...
function pdf(bldgnum, byear, bperiod, utilid, action){
    ...
    if (bldgnum.length == 0) {
       ...
       //     }
       // };
        xmlhttp.open("GET", "consolepdf.asp?portfolio="+pid+"bldgnum="+bldgnum+"&byear="+byear+"&bperiod="+bperiod+"&utilfilter="+utilid+"&action="+action,true);
        xmlhttp.send();
    }
}
...
<table border=0 cellpadding=0 cellspacing=0 width=900 style='border-collapse:
 ...
 <col width=77 style='mso-width-source:userset;mso-width-alt:1718;width:35pt'>
 <col width=39 style='mso-width-source:userset;mso-width-alt:694;width:14pt'>
 <tr height=20 style='height:15.0pt'>
  ...
  <td class=xl1531467 width=127 style='width:75pt'></td>
  <td class=xl1531467 width=99 style='width:34pt'></td>
 </tr>
...
<tr height=28 style='height:21.0pt'>
 ...
 <tr height=20 style='height:15.0pt'>
  ...
  <td class=xl7331467 align=right># Created</td>
  <td class=xl7331467 align=right># Posted</td>
  <td class=xl7331467 align=right># Total</td>
  <td class=xl1531467></td>
  <td class=xl1531467></td>
  <td class=xl1531467></td>
 </tr>
  <%
	...
	dim bldgnum, bldgname, root, pdfdir, link, hasfile, tb, cb, pb, uid, uc, ftime, tbldgnums, ibldgnums, file, tc, pc, cc, util, uicon, last, utilname, newpdfname, have, oldfilefull, newfilefull, blnBillsAvailable
	ibldgnums = 0
	tbldgnums = 0
	tc = 0
	cc = 0
	...
	do until rst1.eof
		...
		do until rst2.eof
			...
			if tonumb(utilfilter)=0 or tonumb(utilfilter) = utilid then
				...
				newpdfname = ucase(bldgnum) &"_"& byear & "." & Right("0" & bperiod, 2) &"_"& utilname & "_TenantBills.pdf"
		...
		%>	
				 <form name=consolepdf action="consolepdf.asp" method="post" target="pdf_pop" onsubmit="window.open('about:blank','pdf_pop','width=10,height=10');">
					 ...
					 <tr height=20 style='height:15.0pt'>
					  <td height=20 class=xl1531467 style='height:15.0pt'></td>
					  <td rowspan=1 class=xl7031467 ><%= bldgnum %></td>
					  <td rowspan=1 class=xl7031467 ><%= left(bldgname,30) %></td>
					  <td class=xl7131467 align=center><img src="<%= uicon %>" alt="<%= uid %>" style="width:15px;height:15px;"></td>
					  <% if tb = 0 then %>
					  ...
					  <% else %>
						  <td class=xl7131467 align=center><% if cb > 0 then %> <%= cb %> <% end if %></td>
						  <td class=xl7131467 align=center><% if pb > 0 then %> <%= pb %> <% end if %></td>
						  <td class=xl7131467 align=center><b><%= tb %></b></td>
						  
					  <% end if %>
					  <td class=xl7231467>
						<% if cb > 0 then %>
							<INPUT type=submit value="Del" onclick="allbills(<%=pid%>,'<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'd');"/>
						<% else %>
							<INPUT type=submit value="Cre" onclick="allbills(<%=pid%>,'<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'c');"/>
						<% end if %>
						<% if pb > 0 then %>
							<INPUT type=submit value="UnP" onclick="allbills(<%=pid%>,'<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'u');"/>
						<% else %>
							<INPUT type=submit value="Post" onclick="allbills(<%=pid%>,'<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'p');"/>
						<% end if %>
						<% if cb > 0 then %>
							<INPUT type=submit value="Gen" onclick="allpdfs(<%=pid%>,'<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'gen');"/></td>
						<% else %>
...
%>
 <tr height=20 style='height:15.0pt'>
  <td height=20 class=xl1531467 style='height:15.0pt'></td>
  <td class=xl1531467>&nbsp;</td>
  <td class=xl1531467><%= tbldgnums %> buildings</td>
...
<% else %>
	<td class=xl1531467><%= ibldgnums %> invoices</td>
<% end if %>  
