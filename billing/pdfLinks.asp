...
function hrsago(ptime)
	...
	if detailed="true" then dname = "_Detailed" end if
	newpdfname = ucase(cbldg) &"_"& byear & "_" & Right("0" & bperiod, 2) &"_"& replace(utilname," ","_") & dname &"_Tenant_Bills.pdf"
	for i = 1 to 1
			strFileName	= "\\serv-pdf\pdfmaker\" & portfolio & "\" & ucase(cbldg) & "\" & ucase(cbldg) & byear & bperiod & utilityid & i & ".pdf"
			PDFName = ucase(cbldg) & byear & bperiod & utilityid & i & ".pdf"
			'response.write detailed
			if detailed="true" then PDFName = "D_"&PDFName end if
			absfile = portfolio & "\" & ucase(cbldg) & "\" & ucase(cbldg) & byear & bperiod & utilityid & i & ".pdf"
'response.write root&pdfdir&PDFName&"</br>"
...
'response.write newpdfname&"</br>"			
			if fso.fileexists(root&pdfdir&newpdfname) and not have and not postback then
				have = true
				blnBillsAvailable = True
				set f = fso.getfile(root&pdfdir&newpdfname)
				ftime = hrsago(f.datelastmodified)
			%>
			<tr><td>&nbsp;</td></tr>
			<tr><td Colspan=4><b>PDF Available:</b>
			<a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%=pdfdir%><%=newpdfname%>?dt=<%=ctime%>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'"><b><%=newpdfname%></b></a> 
			</td></tr>
			<tr><td Colspan=4><b>File created:&nbsp;<%= ftime %> </td></tr>
			
			<%			
			Elseif not postback then %>
			
				<% if fso.fileexists(root&pdfdir&pdfname) then 
						set f = fso.getfile(root&pdfdir&PDFName)
						ftime = hrsago(f.datelastmodified)
				%>
				<tr><td>&nbsp;</td></tr>
				<tr><td Colspan=4><b>PDF Available:</b>
				<a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%=pdfdir%><%=pdfname%>?dt=<%=ctime%>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'"><b><%=pdfname%></b></a> 
				</td></tr>
				<tr><td Colspan=4><b>File created:&nbsp;<%= ftime %> </td></tr>
				<% end if %>
				<tr><td Colspan=2>
	...
	If IspostBack = "true" then
		If PdfRequest <> "true" then
			...
			cmd.Parameters.Append prm
			Set prm = cmd.CreateParameter("detailed", adBOOLEAN, adParamInput)
			cmd.Parameters.Append prm
				...
				cmd.parameters("detailed") = detailed
