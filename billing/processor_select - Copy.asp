  ...
  do until rst1.eof
    ...
    if rst1("post")="True" then posted=true%>
		  ...
		  <%end if%>
            <td>
				<%=rst1("billingname")%>
					<%if rst1("tcount") <> "0" then %> 
						<font color="#FF0000">[<a href="#" onClick="window.open('/genergy2_intranet/itservices/ttracker/troublesearch.asp?pid=<%=pid%>&searchstring=<%=rst1("billingid")%>&action=Search&searchbox=false&accounts=True','SearchNotes','width=800,height=400, scrollbars=no')"><%=rst1("tcount")%> critical ticket(s) still open</a>]</font
					><%end if%>
