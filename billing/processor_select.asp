...
dim billCount,pdfLinker, logoh, logow, strSQL, emailList, corrections, metersaccepted, thisdate, managingcompany, logob, logobw, logobh
...
if trim(building)<>"" then
	rst1.open "SELECT location, b.bldgnum,billurl,logo, logoh, logow, logob, logobw, logobh, managingcompany FROM buildings b, portfolio p, billtemplates bt WHERE b.portfolioid=p.id AND bt.id=p.templateid AND bldgnum='"&building&"'", cnn1
...
<head>
<script>
function importlefrakadjustments(pid)
{
	cWin = window.open('/genergy2/importers/importlefrakadjustments.asp?pid=<%=pid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&utilid=<%=utilityid%>','Import Lefrak Bill Adjustments','width=900,height=1000,resizable=yes, scrollbars=yes')
	cWin.focus();
}
function importcredits(pid)
{
	cWin = window.open('/genergy2/importers/importcredits.asp?bldgnum=<%=building%>&byear=<%=byear%>&bperiod=<%=bperiod%>&utilid=<%=utilityid%>','Import Credits/Adjustments','width=900,height=1000,resizable=yes, scrollbars=yes')
	cWin.focus();
}
function processbills(pid)
{
	cWin = window.open('/genergy2/billing/processbills.asp?pid=<%=pid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&utilfilter=<%=utilityid%>','Process Bills','width=1200,height=1000,resizable=yes, scrollbars=yes')
	cWin.focus();
}
function generatemetercostreport(pid)
{
	cWin = window.open('/genergy2/billing/lefrakcoldwaterreport.asp?pid=<%=pid%>&email=no&byear=<%=byear%>&bperiod=<%=bperiod%>&utilityid=<%=utilityid%>','Residential Cold Water Report','width=650,height=900,resizable=yes, scrollbars=yes')
	cWin.focus();
}
function tenantsummaryexportR(pid)
{
		cWin = window.open('ExportTenantSummary.asp?action=IBS&pid=<%=pid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&y=&building=<%=building%>&utilityid=<%=utilityid%>&btype=R','Residential Cold Water Charges','width=650,height=900,resizable=yes,scrollbars=yes');
		cWin.focus();
}
function tenantsummaryexportC(pid)
{
		cWin = window.open('ExportTenantSummary.asp?action=IBS&pid=<%=pid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&y=&building=<%=building%>&utilityid=<%=utilityid%>&btype=C','Commercial Cold Water Charges','width=650,height=900,resizable=yes,scrollbars=yes');
		cWin.focus();
}
</script>
  ...
  <form name="form1" action="billprocess.asp">
    ...
    if trim(utilityid)<>0 then%>
      ...
      %>
			...
			<% if allowGroups("IT Services,PACorrectionBilling") Then %>
    ...
    <tr bgcolor="#dddddd"> 
      <td colspan="1"> 
	  	<select name="actions" onChange="warningMsg(this);">
			...
			<option value="TenantSummaryExcel">Tenant Summary Excel</option>
		</select>
	  ...
	  <td>
			<% if pid=1179 then %>
					<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="importcredits(<%=pid%>);" class="mgmtlink">Import Credits/Adjustments</a>
			<% elseif pid=1171 then %>
					<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="importlefrakadjustments(<%=pid%>);" class="mgmtlink">Import Lefrak Bill Adjustments</a>
			<% end if %>
					<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="processbills(<%=pid%>);" class="mgmtlink">Process All Bills</a>
			<% if pid = 1171 or pid=163 then %>
					<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="generatemetercostreport(<%=pid%>);" class="mgmtlink">Generate Cold Water Report</a>
					<img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="tenantsummaryexportR(<%=pid%>);" class="mgmtlink">Output Cold Water Charges</a>
					<br>
			<% else %>
		</td>
			<td>&nbsp;</td>
			<% end if %>
				...
				<tr>
			<td width="50%"><h2> Managed by: <%= managingcompany %> </h2></td>
		  </tr>
    <tr> 
      <td colspan="2"> 
	  &nbsp;
    ...
    if not rst1.eof then bperiod=rst1("billperiod")
		'if building = "RNEEMBKA" then
			...
			'response.write byear&":"&bperiod&":"&pdate&"</br>"
  ...
  do until rst1.eof
    ...
    if rst1("post")="True" then posted=true%>
		  ...
		  <%end if%>
            <td>
				<%=rst1("billingname")%>
					<%if rst1("tcount") <> "0" then %> <font color="#FF0000">[<a href="#" onClick="window.open('/genergy2_intranet/itservices/ttracker/troublesearch.asp?pid=<%=pid%>&searchstring=<%=rst1("billingid")%>&action=Search&searchbox=false&accounts=True','SearchNotes','width=800,height=400, scrollbars=no')"><%=rst1("tcount")%> critical ticket(s) still open</a>]</font><%end if%>
					<% if rst1("paperless") then %>
						<img src="images/paperless.png" width="24" height="24" border="0"> 
					<% end if %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<% if rst1("leaseutilityid") = 26941 then %>
						<input type="button" name="action" value="Edit Bill" onClick="return editBill('<%=rst1("leaseutilityid")%>');" id="f9">
					<% end if %>
...
billCount = Math.ceil(Math.ceil(billCount/250)*3);
...
function editBill(itemlid){
  var note = prompt("Please enter $$ value");
  if (note == "" || note == null || note == "undefined") 
    return false;
  else {
    document.form1.lid.value=itemlid;
    document.location = "billprocess.asp?pid=<%=pid%>&building=<%=building%>&bperiod=<%=bperiod%>/<%=byear%>&utilityid=<%=utilityid%>&lid=" + itemlid +"&historic="+document.form1.historic.value+"&action=EditBill&note="+note;
    }
}
...
function billAction(){
	...
	if(frm.actions.value=="View All Detailed Bills"){
		window.open('loading.asp?url=<%=server.urlencode("pdfLinks.asp?genergy2=true&devIP="&request.ServerVariables("SERVER_NAME")&"&billurl="&billurl&"&pid="&pid&"&logo="&logo&"&logoh="&logoh&"&logow="&logow&"&byear="&byear&"&bperiod="&bperiod&"&y=&building="&building&"&bldg="&building&"&b="&building&"&maxPageCount="&maxPageCount&"&detailed=true&billCount="&billCount&"&utilityid="&utilityid)%>','','width=600,height=500,resizable=yes,scrollbars=yes');
	}else if(frm.actions.value=="View [utility only] Bills"){
		window.open('loading.asp?url=<%=server.urlencode("PdfLinks.asp?pid="&pid&"&byear="&byear&"&bperiod="&bperiod&"&utilityid="&utilityid&"&building="&building&"&detailed=false")%>','','width=600,height=500,resizable=yes,scrollbars=yes');
	//}else if(frm.actions.value=="View [utility only] Bills"){
...
//modified by kCheng 4/2/2009
		...
		window.open('loading.asp?url=<%=server.urlencode("/genergy2/UMreports/tenantBillSummary.asp?pid="&pid&"&building="&building&"&syear="&byear&"&utilityid="&utilityid)%>','','width=800,height=900,scrollbars=yes');
	...
	}else if(frm.actions.value=="TenantSummaryExcel"){
		window.open('loading.asp?url=<%=server.urlencode("/genergy2/eri_th/meterservices/TenantSummaryExcel.asp?pid="&pid&"&bldgnum="&building&"&syear="&byear&"&utilityid="&utilityid)%>','','width=800,height=600,scrollbars=yes');
