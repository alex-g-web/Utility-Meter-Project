...
'end if
	...
	dim building, action, zipname, zipfull, ziplink, zipnamepless, zipfullpless, ziplinkpless, utilfilter, utilbills
...
{
	cWin = window.open('/genergy2/billing/lefrakcoldwaterreport.asp?pid=<%=pid%>&email=no&byear=<%=byear%>&bperiod=<%=bperiod%>&utilityid=3','Residential Cold Water Report','width=650,height=900,resizable=yes, scrollbars=yes')
	cWin.focus();
}
function tenantsummaryexportR(pid)
{
		cWin = window.open('ExportTenantSummary.asp?action=IBS&pid=<%=pid%>&byear=<%=byear%>&bperiod=<%=bperiod%>&y=&building=<%=building%>&utilityid=3&btype=R','Residential Cold Water Charges','width=650,height=900,resizable=yes,scrollbars=yes');
		cWin.focus();
}
function insertAdjustments()
{
		cWin = window.open('http://um.cplgroupusa.com/','Load Adjustments File','width=650,height=900,resizable=yes,scrollbars=yes');
		cWin.focus();
}
...
<table border=0 cellpadding=0 cellspacing=0 width=1045 style='border-collapse:
 ...
 <form name=load action="<%= page %>"> 
	 ...
	 <tr height=25 style='height:18.75pt'>
	  ...
	  <td colspan=2 class=xl6812316><input size="1" class=box name="bperiod" value="<%=bperiod%>" /><input  size="2" class=box name="byear" value="<%=byear%>" /></td>
	  <td class=xl1512316></td>
	  <td class=xl1512316></td>
	  <td class=xl1512316></td>
	  <td class=xl1512316></td>
...
<form target="bill_pop" name=bills action="processBillsConsole.asp" onsubmit="window.open('about:blank','bill_pop','width=300,height=500');">
	 <tr height=25 style='height:18.75pt'>
	  ...
	  <td colspan=2 class=xl6712316 align=right>
	  <% if pid = 1171 then %>
	 <b><img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="generatemetercostreport(<%=pid%>);" class="mgmtlink">Generate Cold Water Report</a></b>
	  <% end if %>
	  </td>
	  <td class=xl6712316 align=right><b>Process</b></td>
...
<form name=processBillsConsole action="processBillsConsole.asp" method="post" target="pdf_pop" onsubmit="window.open('about:blank','pdf_pop','width=10,height=10');">	  
	 <tr height=25 style='height:18.75pt'>
	  ...
	  <td colspan=2 class=xl6712316 align=right>
		<% if pid = 1171 then %>
		  <b><img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="tenantsummaryexportR(<%=pid%>);" class="mgmtlink">Output Cold Water Charges</a></b>
		  <% else %>
			&nbsp;
		  <% end if %>
	  </td>
	  <td class=xl6712316 align=right><b>All Bills</b></td>
	  <td class=xl6712316></td>
	  <td class=xl6812316><INPUT type=submit name="action" value="Bills" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'genbills');"/></td>
	  <td class=xl6812316><INPUT type=submit name="action" value="ZipPDF" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'zipbills');"/></td>
	  <td class=xl6812316></td>
	  <td class=xl6812316><INPUT type=submit name="action" value="Summary" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'gensummary');"/></td>
	  <td class=xl6812316><INPUT type=submit name="action" value="ZipSum" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'zipsummary');"/></td>
	  <td class=xl6712316></td>
	  <td class=xl6712316></td>
	  <td class=xl7312316></td>
	  <td class=xl7312316></td>
 ...
 <tr height=25 style='height:18.75pt'>
  ...
  <td class=xl6712316>
	<% if pid = 1171 then %>
	 <b><img src="images/aro-rt.gif" align="absmiddle" hspace="2" border="0"><a href="#" onClick="insertAdjustments();" class="mgmtlink">Insert Bill Adjustments</a></b>
	<% end if %>
  </td>
  <td class=xl6712316></td>
  <td class=xl6712316></td>
<td class=xl6812316><INPUT type=submit name="action" value="f/Mail" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'genbills');"/></td>  
<td class=xl6812316><INPUT type=submit name="action" value="eMail" onclick="allpdfs(<%=pid%>,'-',<%=byear%>, <%=bperiod%>,<%=utilfilter%>, 'genbills');"/></td>
  <td class=xl6712316></td>
  <td class=xl6712316></td>
  <td class=xl6712316><span id="sumtime"></span></td>
  <td class=xl6712316></td>
  <td class=xl6712316></td>
  <td class=xl7312316></td>
  <td class=xl7312316></td>
 ...
 <tr height=25 style='height:18.75pt'>
  ...
  <td class=xl7312316></td>
   ...
   <%
	...
	dim bldgnum, bldgname, root, pdfdir, link, hasfile, tb, cb, pb, uid, uc, ftime, tbldgnums, totalbills, file, tc, pc, cc, util, uicon, last, utilname, newbillsname, have, oldfilefull, newbillsfile, blnBillsAvailable, newsummaryname, newsummaryfile, totalsummary, sumtime, pdftime, ad
	totalbills = 0
	totalsummary = 0
	tbldgnums = 0
	tc = 0
		...
		do until rst2.eof
			...
			if tonumb(utilfilter)=0 or tonumb(utilfilter) = utilid then
				...
				sql = "SELECT "& _
					...
					"(SELECT count(*) FROM tblbillbyperiod WHERE reject=0 and totalamt is null and bldgnum='"&bldgnum&"' and billperiod="&bperiod&" and billyear="&byear&" and utility="&utilid&") as billserrored, (select count(*) from tblbillbyperiod WHERE adjusted=1 and bldgnum='"&bldgnum&"' and billperiod="&bperiod&" and billyear="&byear&" and utility="&utilid&") as adjusted"
				...
				newbillsname = ucase(bldgnum) &"_"& byear & "_" & Right("0" & bperiod, 2) &"_"& replace(utilname, " ", "_") & "_Tenant_Bills.pdf"
				newsummaryname = ucase(bldgnum) &"_"& byear & "_" & Right("0" & bperiod, 2) &"_"& replace(utilname, " ", "_") & "_Bill_Summary.pdf"
				oldfilefull = root&pdfdir&PDFName
				'response.write oldfilefull&"</br>"
				
				'response.write utilname&"</br>"
				newbillsfile = root&pdfdir&newbillsname
				newsummaryfile = root&pdfdir&newsummaryname
				'response.write newbillsfile&"</br>"
	...
	<form name=processBillsConsole action="processBillsConsole.asp" method="post" target="pdf_pop" onsubmit="window.open('about:blank','pdf_pop','width=10,height=10');">
		 <tr height=25 style='height:18.75pt'>
		  ...
		  <% if tb = 0 then %>
			  ...
			  <td class=xl7212316>&nbsp;</td>
		  <% else %>
			  <% if (tb-cb)/tb > .2 then %> <td class=xl7212316 style="color:red; font-weight:bolder"> <% else %> <td class=xl7212316><%end if%><% if ad > 0 then %>(<%= ad %>) <% end if %> <% if cb > 0 then %> <%= cb %> <% end if %></td>
...
<%
	...
	zipname = replace(utilname, " ", "_") & "_Tenant_Bills"+"_"& byear & "_" & Right("0" & bperiod, 2) &".zip"
	zipfull = root&p&"/"&zipname
	ziplink = p&"/"&zipname & "?dt=" & ctime
	
	if tonumb(utilfilter)=0 then utilname = "All_Utilities" end if
	zipnamepless = replace(utilname, " ", "_") & "_Tenant_Bills"+"_"& byear & "_" & Right("0" & bperiod, 2) &"paperless.zip"
	zipfullpless = root&p&"/"&zipnamepless
	ziplinkpless = p&"/"&zipnamepless & "?dt=" & ctime

	response.write zipname&"</br>"
	response.write zipnamepless&"</br>"
%>
	<td colspan=2 class=xl7012316>
		<% if CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&p&"/"&zipname) then 
			set f = fso.getfile(zipfull)
			pdftime = hrsago(f.datelastmodified)
			set f = nothing
		%>  
				<div id="bills.link"><a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%= ziplink %>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'" download><img src="images\zip.png" alt="<%= zipname %> "style="width:15px;height:15px;">&nbsp;<%= totalbills %> Tenant Bills </a></div>
				<%= pdftime %>
		 <% end if %>
		<% if CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&p&"/"&zipnamepless) then 
			set f = fso.getfile(zipfullpless)
			pdftime = hrsago(f.datelastmodified)
			set f = nothing
		%>  
				<div id="bills.link"><a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%= ziplinkpless %>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'" download><img src="images\zip.png" alt="<%= zipnamepless %> "style="width:15px;height:15px;">&nbsp;<%= totalbills %> Tenant Bills Paperless</a></div>
				<%= pdftime %>
		 <% end if %>
		 
  </td>
 <% if CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&p&"/"&zipname) and CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&p&"/"&zipnamepless) then %>
 <% else %>
	<td colspan=2 class=xl7012316><%= totalbills %> Tenant Bills</td>
 <% end if %>	
  
<%
	if tonumb(utilfilter)=0 then utilname = "All_Utilities" end if
	zipname = byear & "_" & Right("0" & bperiod, 2) &"_"& replace(utilname, " ", "_") & "_Bill_Summary.zip"
	zipfull = root&p&"/"&zipname
	ziplink = p&"/"&zipname & "?dt=" & ctime
...
<% if CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&p&"/"&zipname) then 
	set f = fso.getfile(zipfull)
	sumtime = hrsago(f.datelastmodified)
%>  
  <td colspan=2 class=xl7012316>		
		<div id="bills.link"><a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfMaker/<%= ziplink %>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'" download><img src="images\zip.png" alt="<%= zipname %> "style="width:15px;height:15px;">&nbsp;<%= totalbills %> Bill Summaries </a></div>
		
		<%= sumtime %>
  </td>
 <% else %>
	<td colspan=2 class=xl7012316><%= totalsummary %> Bill Summaries</td>
 <% end if %>
  <td class=xl7312316></td>
 </tr>
 <tr height=25 style='height:18.75pt'>
...
link.href='https://appserver1.genergy.com/pdfMaker/<%= ziplink %>'
link.innerText='file'
document.getElementById('pdftime').appendchild(link)
</script>
</html>
<% set f=nothing
set fso=nothing %>