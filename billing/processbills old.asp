...
function allpdfs(bldgnum, byear, bperiod, utilid, action){
	var opts = {
	  lines: 20, // The number of lines to draw
	  length: 0, // The length of each line
	  width: 6, // The line thickness
		...
		%>	
				 <form name=processBillsConsole action="processBillsConsole.asp" method="post" target="pdf_pop" onsubmit="window.open('about:blank','pdf_pop','width=10,height=10');">
					 ...
					 <tr height=20 style='height:15.0pt'>
					  ...
					  <% end if %>
					  <td class=xl7231467><INPUT type=submit value="Generate" onclick="allpdfs('<%=bldgnum%>',<%=byear%>, <%=bperiod%>,<%=utilid%>, 'gen');"/></td>
					<%
					if CheckRemoteURL("https://appserver1.genergy.com/pdfMaker/"&pdfdir&newpdfname) then
						have = true
