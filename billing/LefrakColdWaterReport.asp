...
<%end if
	...
	Dim uSql, objFSO, ctime, bperiodchar, csvPortfolio, csvUtility, csvPeriod, crlf, pbpath, filePath, csvFile, csvHeaders, tMeter, csvUtil, PcsvFile, resfile, ftpresfile, putf, ppath
	...
	'	if not rst1.eof then
			...
			case else
				...
				csvPeriod = Billyear & "_" & ZeroPadInteger(Billperiod,2)
				dailyPeriod = Billyear & "_" & ZeroPadInteger(Billperiod,2) & "-" & ZeroPadInteger(day(now),2)
				...
				exportRoot = "\\serv-pdf\pdfmaker"
				ftproot = "d:\FTP\LeFrakData\Export Files\"
				filePath = exportRoot & pbpath
				csvFile = ucase(building) & "_" & csvPeriod & "_" & csvUtility & "_" & "Report" & ".csv"
				if portfolioid = 1171 then
					PcsvFile = "Residential" & "_" & csvUtility & "_" & "Report" & "_" & csvPeriod &  ".csv"
				else
					PcsvFile = csvUtility & "_" & "Report" & "_" & csvPeriod & ".csv"
				end if
  ...
  Next
		...
		'Do Until rst1.eof
			...
			do until rst2.eof
			   ...
			   'response.end
				...
				'	end if
				'else
					cUsage = rst2("currentused")'/usagedivisor,2) 
					pUsage = rst2("previousused")'/usagedivisor,2) 
					totalKWH 	= totalKWH + formatnumber(cdbl(cUsage))
				'end if 
				'
	...
	'		loop
			...
			If objFSO.FileExists(filePath & csvFile) Then 
				...
				<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="https://appserver1.genergy.com/pdfmaker/<%=pbpath%><%=csvFile%>"&"?dt="&ctime target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=csvFile%></b></a> 
				</br>
				<%
			end if
		'else
	...
	PBinaryStream.SaveToFile ftpresfile, adSaveCreateOverWrite
	PBinaryStream.Flush
	PBinaryStream.Close
	...
	If objFSO.FileExists(resfile) Then 
		...
		<a style="font-family:arial;font-size:12;text-decoration:none;color:black;" href="https://appserver1.genergy.com/pdfmaker/<%=ppath%><%=pcsvFile%>"&"?dt="&ctime target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'black'"><b><%=pcsvFile%></b></a> 
		</p>
		<%
	Else
		%>
