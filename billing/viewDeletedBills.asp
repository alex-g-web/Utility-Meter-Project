...
function viewBill(pid, billid, rstlid, ypid, detailed, utility, buildpdf){
	buildpdf = (buildpdf!=false?true:false);
  var url = 'http://pdfmaker.genergyonline.com<%=billlink%>genergy2=true&devIP=<%=request.ServerVariables("SERVER_NAME")%>&building=<%=building%>&lid=' + rstlid + '&byear=<%=byear%>&bperiod=<%=bperiod%>&y='+ypid+'&ypid='+ypid+'&l=' + rstlid+'&detailed='+detailed+'&utilityid='+utility+'&buildpdf='+buildpdf+'&billid='+billid+'&pid='+pid;
  //alert(url);
...
<%do until rst1.eof%>
	<tr style="background-color:white">
		<td><a href="javascript:viewBill(<%= pid %>,<%=rst1("id")%>,'<%=rst1("leaseutilityid")%>', '<%=rst1("ypid")%>','true', <%=rst1("utilityid")%>, true);"><img src="images/pdf_bill.gif" width="21" height="22" border="0"></a><br><!--</td>-->
		<a href="javascript:viewBill(<%= pid %>,<%=rst1("id")%>,'<%=rst1("leaseutilityid")%>', '<%=rst1("ypid")%>','true', <%=rst1("utilityid")%>, false);">HTML</a></td>
