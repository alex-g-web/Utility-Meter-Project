...
End Function
	...
	newpdfname = ucase(cbldg) &"_"& byear & "." & Right("0" & bperiod, 2) &"_"& utilname & "_TenantBills.pdf"
	for i = 1 to maxPageCount
			...
			if CheckRemoteURL("http://pdfmaker.genergyonline.com/pdfMaker/"&pdfdir&newpdfname) and not have then
				have = true
				blnBillsAvailable = True
			%>
			<tr><td Colspan=2>
			<a style="font-family:arial;font-size:12;text-decoration:none;" href="http://pdfmaker.genergyonline.com/pdfMaker/<%=pdfdir%><%=newpdfname%>?dt=<%=ctime%>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'"><b><%=i%> page Bills pdf: <%=newpdfname%></b></a> 
			</td></tr>
			<%
			Else %>
				<tr><td Colspan=2>
...
'Response.Write("bldgId: " + building + ", billyear: " + byear + ", bp: " + bperiod + ", utilityId: " + utilityid)
			...
			***REMINDER---use F5 option when viewing re-generated version of PDF***</td>
			<%else%>
			<td Colspan=4><%= ucase(cbldg) %> | address goes here <INPUT type=submit value="(Re)Generate PDFs"></INPUT></td>
