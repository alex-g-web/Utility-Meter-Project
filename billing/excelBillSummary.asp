...
<%end if
	...
	case else
		...
		dailyPeriod = Billyear & "_" & ZeroPadInteger(Billperiod,2) & "_" & ZeroPadInteger(day(now),2)
		
		uSql = "select replace(utilitydisplay,' ','_') as utilitydisplay from tblutility where utilityid=" & UtilityId
		...
		csvFile = building & "_" & csvPeriod & "_" & csvUtility & "_" & "Average_Meter_Cost_Report" & ".csv"
		dailyFile = building & "_" & dailyPeriod & "_" & csvUtility & "_" & "Average_Meter_Cost_Report" & ".txt"
...
Do Until rst1.eof
	...
	do until rst2.eof
	   ...
	   'response.end
		...
		'response.write rst1("billingname") & "</br>" 						't name
	...
	'
	for i = 0 to tMeter-1
		if tUsage = 0 then tUsage = 1 end if
...
if not rst1.eof then
	...
	iBldgTotalsRow = iBldgTotalsRow + 1 
	if utilityid=3 or utilityid=10  or utilityid=4 then 
	...
	<table>
	    ...
	    <td>
			...
			<a style="font-family:arial;font-size:12;text-decoration:none;" href="https://appserver1.genergy.com/pdfmaker<%= pbPath %><%= billsummaryFile %>" target="_blank" onMouseOver="this.style.color= 'lightblue'"; onMouseOut="this.style.color = 'blue'"><b><%= billsummaryFile %> </b></a> 
	    </td>
	    </tr>
	</table>
	<%
	    ...
	    If Len(strMailingList) >= 0 then
			...
			With Mail
				...
				.To = "rb@cplgroupusa.com"
				.From = "filestore@cplgroupusa.com"
