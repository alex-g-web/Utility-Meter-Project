...
dim leaseid, ypid, building, pid, byear, bperiod, utilityid, detailed, meterbreakdown, calcintpeak, SJPproperties, ShowUsageDetails, ShowDemandDetails,onlinereview,totdeliverychrgs, maxmeters, textheader, masterTotal,demo, reject, energydetail, paperless, asql, forManagement, logoh, logow, logo
dim pdfsession,startCount,endCount, pdf, tenantnum
leaseid = trim(Request("l"))
ypid = trim(request("y"))
utilityid = trim(request("utilityid"))
building = trim(request("building"))
'response.write("<BR>"+building)
'building = "260 Mad"
pid = trim(request("pid"))
byear = trim(request("byear"))
bperiod = trim(request("bperiod"))
logo = trim(request("logo"))
logow=trim(request("logow"))
logoh=trim(request("logoh"))
...
dim cnn1, rst1, rst2, rst3, rst4, bldgrs, usagelabel, sql, notabill
...
asql = "select case when (select bldgnum from DailyExportBuildings where bldgnum = '"&building&"') is not null then 1 else 0 end as notabill"
rst4.open asql, cnn1
notabill = rst4("notabill")
rst4.close



dim templid, tempypid, temputility, extusage
...
if trim(request("billid"))<>"" and trim(leaseid)<>"" then
	temputility=utilityid
	printBill leaseid, trim(request("billid")), temputility,extusage
elseif leaseid<>"" and ypid<>"" then
  ...
  if not bldgrs.eof then printBill leaseid, bldgrs("billid"), utilityid,extusage
	bldgrs.close
elseif building<>"" then
	if ypid<>"" then
		...
		do until bldgrs.eof
			...
			asql = "select isnull(paperless,0) as paperless from tblLeases where billingid=(select billingid from tblLeasesUtilityPrices where leaseutilityid=(select leaseutilityid from tblBillByPeriod where id="&bldgrs("billid")&"))"
			
			rst4.open asql,cnn1
			if not rst4.eof then paperless = rst4("paperless")
			rst4.close
			
			asql = "select top 1 forManagement from mailer where pid="&pid&" and bldgnum='"&building&"' order by printdate desc"
			
			rst4.open asql,cnn1
			if not rst4.eof then forManagement = rst4("forManagement")
			rst4.close
			
			if paperless = 0 or forManagement=1 or forManagement = "True" then				
				printBill templid, bldgrs("billid"), temputility, extusage
			end if
			'response.Write("here: ForManagerment - "& forManagement)
			bldgrs.movenext
		loop
    ...
    sql = sql & "  ORDER BY TenantName"
		...
		'on Error resume next
			...
			'response.Write("here2: ForManagerment - "& forManagement)
			bldgrs.movenext
		loop
	end if
end if
...
'###
'### begin of ispaperless check
'###
sub isPaperless(leaseid, billid)
	asql = "select isnull(paperless,0) as paperless from tblLeases where billingid=(select billingid from tblLeasesUtilityPrices where leaseutilityid=(select leaseutilityid from tblBillByPeriod where id="&bldgrs("billid")&"))"
...
'###
'### begin of bill chooser
'###
sub printBill(lid, billid, utilid, ext)
'response.write billid
	if ( pid = 1171 or building = "2288Atlant" ) and utilityid = 3 then
		showtenantbill_H2O lid, billid, utilid, ext
		'showtenantbill_H2O templid, bldgrs("billid"), temputility, extusage
	else
		'showtenantbill_genergy templid, bldgrs("billid"), temputility, extusage
		showtenantbill_genergy lid, billid, utilid, ext
	end if
...
'### begin of showtenantbill_H2O, to line:1647 ###

sub showtenantbill_H2O(leaseid, billid, utilityid, extusage)
	...
	<table width="102%" cellpadding="2" cellspacing="0" border="0" height="10%">
	<tr><td valign="top">
	<%mailer rst2("tenantname"), rst2("tenantnum"), rst2("billingaddress"), rst2("tcity"), rst2("tstate"), rst2("tzip"), rst2("btbldgname"), rst2("billingaddress"), rst2("btcity"), rst2("btstate"), rst2("billzip"), rst2("onlinebill"), textheader,demo, paymentterm, rst2("bldgnum")%>
	</td></tr></table>

	<table width="102%" cellpadding="2" cellspacing="0" border="0" height="75%">
	<%if textheader="advRealty.asp" then%>
	<tr><td align="center"><!--#INCLUDE FILE="advRealty.asp" --></td></tr>
	<%end if%>
	<tr><td 
	<%if (lcase(trim(serviceclass))="lpls2" and pid <> "15") or serviceclassid= 36 then%><%else%>height="30%"<%end if%> valign="top">
	<!-- meterlisting -->
	...
	<tr>
		<td colspan="2" width="256" bgcolor="#eeeeee" align="center"><%= rst2("billyear") & right("0"&rst2("billperiod"),2)&right("0"&rst2("unittype"),2)&"."& ucase(rst2("tenantnum")) %></td>
	...
	case else
	  if rst2("calcintpeak") then calcintpeak = true else calcintpeak = false
		  if extUsage then 
			  ...
			  metertableheads_H2O rst2("billyear"), rst2("billperiod"), rst2("datestart")-1, rst2("dateend"), calcintpeak
		  end if
	end select
	Set rst1 = Server.CreateObject("ADODB.recordset")
					...
					if meterbreakdown then
						...
						<tr bordercolor="#FFFFFF">
							...
							<%if not hasShownMulti_and_Num then%>
								...
								<td align="center"><%=Formatnumber(rst1("manualmultiplier"),2)%></td>
							<%end if%>
	...
	<tr bordercolor="#FFFFFF">
		...
		<td align="right" colspan="3"><b><%=Formatnumber(tot_kwhused,2)%> CCF</b></td>       
	...
	<tr bordercolor="#FFFFFF">
		...
		<!--Michelle T. 5/29/2009.   commented out to eliminate subtotal detail on a meter level-->
		<td align="center">&nbsp;</td>
		<td align="right" colspan="3"><b><%=Formatnumber(tot_kwhused*748.052,2)%> Gallons</b></td>       
		<%'if utilityid <> 1 and utilityid <> 4 then%> 
		<!--commented line below to accomodate for subtotal not calculating correctly, as per kimberly formula should be ((cw+sc)*cff) from water bill. Michelle T. 5/27/2009 -->
	   <td align="right"><b><%'if coincidentflag then response.write formatcurrency(FormatNumber(tot_demand_C,2)) else response.write formatcurrency(FormatNumber(tot_demand_P,2))%></b></td>
	   <td align="right"><b><%'if coincidentflag then response.write formatcurrency(FormatNumber(tot_demand_C,2)) else response.write formatcurrency(((cdbl(rst2("energydetail"))+cdbl(rst2("demanddetail"))) * tot_kwhused),2)%></b></td>
	...
	<tr>
	  <td bgcolor="#eeeeee" valign="top">
		...
		if ((utilityid = 6 OR utilityid = 21) and building = "1992B") then cUnit = "KBTU" end if 'rsm
		if ((utilityid = 6 OR utilityid = 21) and building = "1992B") then dUnit = "KBTU" end if 'rsm
 ...
 if rst2("adjusted") then
	subTotal = cdbl(rst2("subtotal") )

	  	%>
			<td align="right" colspan=4><b>Discounted Bill</b></td>
			<td width="20">&nbsp;</td>
			<td align="right">&nbsp;</td>			
		  </tr><tr>
			<td align="right" colspan=2>Original Bill Amount </td>
			<td width="20">&nbsp;</td>
			<td align="right"><%=FormatCurrency(rst2("originaltotal"),2)%></td>			
		  </tr><tr>		  

		<td align="right" colspan=2>Discounted Bill Amount</td>
		<td width="20">&nbsp;</td>
...
%>	 
	...
	<%case else
	  ...
	  %>
		<td align="right" colspan=2>Sub Total</td>
		<td width="20">&nbsp;</td>
		<td align="right"><%=FormatCurrency(rst2("subtotal1"),2)%></td>
	  </tr>
		  <%if SJPproperties then%>
			<tr>
	...
	<tr height="175"><td valign="top">
	<%
	dim hidedemand, sbldg
	...
	<tr>
		<% if tenantnum <>"" then %>
		<td colspan="3">&nbsp;<br><center><br>
			<% if pid = 994959949 then %>
				<h1>Resolve to reduce clutter. Go paperless.</h1></br>
				<h3>View online bill and register a valid email at cplgroupusa.com/login-system/ </br>with access code </h3>
			<% else %>
				To view online bill, login to <b>cplgroupusa.com/login-system/</b> with access code </br>
			<% end if %>
			<h3><b><%=tenantnum%>.<%=building%></b></h3>
		</center></td>
		<% end if %>
		</tr>
	</table>
...
'###########################################################################################
sub mailer(tenantname, tenantnum, tstrt, tcity, tstate, tzip, btbldgname, billingaddress, btcity, btstate, btzip, isonlinebill,textheader,demo, paymentterm, bldgnum)%>
...
<%
    ...
    %>
		...
		<% if btcity="" and btstate="" then %><%=""%><% else %><%=rst2("btcity")%>, <%=rst2("btstate")%>&nbsp;<%=rst2("billzip")%><% end if %><br>
	  <% if bldgnum="rneembka" then %> <%=""%> <% else %> <%=paymentterm%> <% end if %></b><br></font>
	  </td>
...
sub metertableheads_H2O(billyear, billperiod, datestart, dateend, showintpeaks)%>
    <%if trim(rst2("tenantname")) = "Davis Polk & Wardwell" then %>
	 <tr bgcolor="#eeeeee">
        <td colspan="3" align="center"><%=trim(rst2("tenantname"))%></td>
    </tr> <%
	end if %>
    <%if rst2("portfolioid") = 1171 then %>
	 <tr bgcolor="#eeeeee">
        <td colspan="3" align="center"><%=trim(rst2("tenantname"))%></td>
        <td colspan="1" align="center">Floor:</td>
        <td colspan="3" align="left"><%=trim(rst2("flr"))%></td>
    </tr> <%
	end if %>
    <tr bgcolor="#eeeeee">
    	...
    	<td align="center">No.&nbsp;Days</td>
		<%if showUsagedetails = false then %><td colspan="2" width="20%" align="center">READINGS in CF</td><%end if%>
...
sub metertableheadsExtUsage(billyear, billperiod, datestart, dateend)%>
    ...
    <tr bgcolor="#eeeeee">
    	...
    	<td align="center">To</td>
    	<td align="center">No.&nbsp;Days</td>
		<td colspan="2" width="20%" align="center">READINGS in CF</td>
...
sub metertableheadsWater(billyear, billperiod, datestart, dateend, utilityid, leaseid)
		...
		if ((Utilityid = 6 OR Utilityid = 21) and (building = "1992B" or building = "1460B")) then usageHeader = "KBTU" end if  'rsm
...
'###
'### begin of showtenantbill_genergy, is rest of file ###
'###
sub showtenantbill_genergy(leaseid, billid, utilityid, extusage)
	...
	case else
		if SJPproperties then
		sql = "SELECT isnull(b.btzip,'') as billzip, b.portfolioid,isnull(r.btstrt,'') as billto, isnull(r.energydetail,'') as energydetail, isnull(r.demanddetail,'') as demanddetail, r.utility as unittype, isnull(Totalamt,0) as Totalamtnonull, isnull(tax,0) as taxnonull, isnull(energy,0) as energynonull, isnull(r.tstrt,'') as billingaddress, r.fueladj as fadj, isnull(demand,0) as demandnonull, isnull(credit,0) as creditnonull, isnull(r.adjustment, 0) as adjustmentnonull, isnull(credit,0) as credit, rt.[type] as rt,rt.id as rtid, datediff(day, datestart,dateend)+1 as days, case rt.[type] when 'AVG Cost 1' then round(avgkwh,6) when 'AVG COST 2' then round(unitcostkwh,6) else ' ' end as akwhdisplay, case rt.[type] when 'AVG COST 2' then round(isnull(tunitcostkw,0),6) else ' ' end as akwdisplay, case when Totalkw=0 then 0 else ((Totalkwh/Totalkw)/(datediff(day, ypiddatestart,ypiddateend)+1)*24) end as loadfactor, isnull(r.adminfee,0) as adminfee, isnull(r.adminfeedollar,0) as adminfeedollar, r.billperiod, r.billyear, r.datestart, r.dateend, isnull(r.servicefee,0) as servicefeenonull,r.addonfee as myaddonfee, r.unit_credit, isnull(r.subTotal,0) as subTotal, r.tenantname, l.flr, lup.calcintpeak, r.*, l.onlinebill FROM rpt_Bill_summary_NoBill r, tblleases l, buildings b, dbo.ratetypes rt, tblleasesutilityprices lup WHERE r.[type]=rt.id AND b.bldgnum=r.bldgnum and lup.leaseutilityid=r.leaseutilityid and l.billingid=lup.billingid and r.billid="&billid
		
		else
		sql = "SELECT isnull(b.btzip,'') as billzip, b.portfolioid,isnull(r.btstrt,'') as billto, isnull(r.energydetail,'') as energydetail, isnull(r.demanddetail,'') as demanddetail, r.utility as unittype, isnull(Totalamt,0) as Totalamtnonull, isnull(tax,0) as taxnonull, isnull(energy,0) as energynonull, isnull(r.tstrt,'') as billingaddress, r.fueladj as fadj, isnull(demand,0) as demandnonull, isnull(credit,0) as creditnonull, isnull(r.adjustment, 0) as adjustmentnonull, isnull(credit,0) as credit, rt.[type] as rt,rt.id as rtid, datediff(day, datestart,dateend)+1 as days, case rt.[type] when 'AVG Cost 1' then round(avgkwh,6) when 'AVG COST 2' then round(unitcostkwh,6) else ' ' end as akwhdisplay, case rt.[type] when 'AVG COST 2' then round(isnull(tunitcostkw,0),6) else ' ' end as akwdisplay, case when Totalkw=0 then 0 else ((Totalkwh/Totalkw)/(datediff(day, ypiddatestart,ypiddateend)+1)*24) end as loadfactor, isnull(r.adminfee,0) as adminfee, isnull(r.adminfeedollar,0) as adminfeedollar, format(r.billperiod,'00') as billperiod, r.billyear, r.datestart, r.dateend, isnull(r.servicefee,0) as servicefeenonull,r.addonfee as myaddonfee, r.unit_credit, isnull(r.subTotal,0) as subTotal, r.tenantname, l.flr, lup.calcintpeak, r.*, l.onlinebill, isnull(r.subTotal1,0) as subtotal1, isnull(r.externalfee,0) as externalfee FROM rpt_bill_summary r, tblleases l, buildings b, dbo.ratetypes rt, tblleasesutilityprices lup WHERE r.[type]=rt.id AND b.bldgnum=r.bldgnum and lup.leaseutilityid=r.leaseutilityid and l.billingid=lup.billingid and r.billid="&billid
	...
	if (trim(utilityname) = "Cold Water" or trim(utilityname) = "Hot Water") And (pid = 171) Then
		utilityname = "Water"
	End If
				...
				case 3, 10, 6,21,22
					if meterbreakdown then
						...
						<tr bordercolor="#FFFFFF">
							...
							<%'RESPONSE.Write "1:" & rst2("demanddetail") &"<br>"
								'RESPONSE.Write "2" &rst2("energydetail")
response.write 				currentUsage &"</br>"
	...
	<%if detailed="true" and utilityid<>3 and utilityid<>10 and utilityid<>6 and utilityid<>21 and utilityid<>22 then%>
	<table width="640" align="center" border="0">
	<tr><td><%if trim(rst2("energydetail"))<>"" then%><%if split(serviceclass," ")(0) = "ConEd" then%>Supply Charge<%else if ucase(trim(serviceclass)) <> "SC4 RA1" and ucase(trim(serviceclass)) <> "SC4 RA2" then%>Consumption<%else%>Invoice<%end if%><%end if%> Details:<%end if%></td>
		<td><%if trim(rst2("demanddetail"))<>"" then%><%if split(serviceclass," ")(0) = "ConEd" then%>Delivery Charge<%else%>Demand<%end if%> Details:<%end if%></td>
	</tr>
	<tr><td valign="top"><%if rst2("energydetail")<>"" then%><%=replace(replace(rst2("energydetail"),"|","<br>")," ","&nbsp;")%><%end if%></td>
		<td valign="top"><%if rst2("demanddetail")<>"" then%><%=replace(replace(rst2("demanddetail"),"|","<br>")," ","&nbsp;")%><%end if%></td></tr>
	</table>
	...
	<tr>
	  <td bgcolor="#eeeeee" valign="top">
		...
		if (cUnit = "") then cUnit = "Ton/Hr"
		if (dUnit = "") then dUnit = "Ton"
		if ((utilityid = 6 OR utilityid = 21) and (building = "1992B" or building = "1460B")) then cUnit = "KBTU" end if 'rsm
		if ((utilityid = 6 OR utilityid = 21) and (building = "1992B" or building = "1460B")) then dUnit = "KBTU" end if 'rsm
		...
		%>
		  ...
		  <tr bgcolor="#eeeeee">
			<td>Service Fee</td>
			<td width="30">&nbsp;</td>
			<td><%if rst2("SHOWADDONFEE")=1 then %><%=FormatCurrency(rst2("myaddonfee"),2)%>/per meter<%else%><%=FormatCurrency(rst2("servicefeenonull"),2)%><%end if%></td>
		  </tr>
		  <tr bgcolor="#eeeeee">
			<td>Admin Fee</td>
			<td width="30">&nbsp;</td>
		...
		<%case else%>
		  ...
		  <% if rst2("datestart") >= "04/01/2010" and ucase(trim(serviceclass)) = "SC4R1" then %>	
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td>SC9R1<%end if%></td>
		  </tr>
		   <% if rst2("datestart") >= "04/01/2010" and ucase(trim(serviceclass)) = "SC4RA1" then %>	
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td>SC9RA1<%end if%></td>
		  </tr>
		   <% if rst2("datestart") >= "04/01/2010" and ucase(trim(serviceclass)) = "SC4R2" then %>	
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td>SC9R2<%end if%></td>
		  </tr>
		   <% if rst2("datestart") >= "04/01/2010" and ucase(trim(serviceclass)) = "SC4RA2" then %>	
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td>SC9RA2<%end if%></td>
		  </tr>
		  <% if rst2("datestart") >= "04/01/2010" and (ucase(trim(serviceclass)) <> "SC4R1" and ucase(trim(serviceclass)) <> "SC4RA1" and ucase(trim(serviceclass)) <> "SC4R2" and ucase(trim(serviceclass)) <> "SC4RA2") then%>
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td><%=serviceclass%><%end if%></td>
		  </tr>
		  <% if rst2("datestart") < "04/01/2010" then%>
		  <tr bgcolor="#eeeeee">
			<td>Service Class</td>
			<td width="30">&nbsp;</td>
			<td><%=serviceclass%><%end if%></td>
	...
	case 3, 10, 6,21,22		'water utilities%>
	 ...
	 if rst2("adjusted") then
		subTotal = cdbl(rst2("subtotal") )
		 if pdf="False"  then
			%>
				<td align="right" colspan=2>Adjusted Bill by <%= rst2("adjustedby") %> </td>
				<td width="20">&nbsp;</td>
				<td align="right"><%=FormatCurrency(((cdbl(rst2("energydetail"))+cdbl(rst2("demanddetail"))) * formatnumber(tot_kwhused,2)),2)%></td>
					<!--<td align="right"><%=FormatCurrency(rst2("subtotal"),2)%></td>-->
			  </tr><tr>
			<%		
		 end if
	 ...
	 %>
		<td align="right" colspan=2>Base Charge</td>
		<td width="20">&nbsp;</td>
		<td align="right"><%=FormatCurrency(rst2("subTotal"),2)%></td>
			<!--<td align="right"><%=FormatCurrency(rst2("subtotal"),2)%></td>-->
	  </tr>
	  ...
	  <tr>
		<td align="right" colspan=2>Admin Fee</td>
		<td width="20">&nbsp;</td>
		<td align="right"><%=formatcurrency(cdbl(rst2("Adminfeedollar")))%></td>
	  </tr>
	  <%subTotal = subTotal + cdbl(rst2("Adminfeedollar"))%>
	  <tr>
		<td align="right" colspan=2>Service&nbsp;Fee</td>
		<td width="20">&nbsp;</td>
		<td align="right"><%=formatcurrency(cdbl(rst2("serviceFeenonull")))%>
	  ...
	  <tr>
		...
		<td width="20">&nbsp;</td>
		<!--<td align="right"><%=FormatCurrency(rst2("subtotal"),2)%></td>-->
		<td align="right"><%=FormatCurrency(subTotal,2)%>
	  </tr>
	  <tr>
		<td align="right" colspan=2>Sales Tax</td>
		<td width="20">&nbsp;</td>
	...
	<%case else
	  ...
	  subTotal = (cDbl(rst2("energynonull"))+cDbl(rst2("demandnonull")))
	  %>
		<td align="right" colspan=2>Base Electric Charge</td>
		<td width="20">&nbsp;</td>
		<td align="right"><%=FormatCurrency(rst2("subTotal1"),2)%></td>
	  </tr>
	   <tr>
			<td align="right" colspan=2>Other Fee</td>
			<td width="20">&nbsp;</td>
		  <%dim externalfeecalc
		  if trim(rst2("externalfee"))<>"" then  externalfeecalc = cdbl(rst2("externalfee")) else externalfeecalc = 0
		  
		  %> 
			<td align="right"><%=FormatCurrency(externalfeecalc,2)%></td>
	  </tr>	 
	  <tr>
			<td align="right" colspan=2>Admin Fee</td>
			<td width="20">&nbsp;</td>
	...
	<tr><td valign="top">
	<%footer rst2("tenantname"), rst2("tenantnum"), rst2("billingaddress"), rst2("tcity"), rst2("tstate"), rst2("tzip"), rst2("btbldgname"), rst2("billingaddress"), rst2("btcity"), rst2("btstate"), rst2("billzip"), rst2("onlinebill"), textheader,demo, paymentterm, rst2("bldgnum"), notabill%>
	</td></tr>
	</table>
	<%
	'if ucase(trim(serviceclass)) = "LPLS2" and detailed="true" then makeLPLS2totals billid 
...
'###########################################################################################
sub footer(tenantname, tenantnum, tstrt, tcity, tstate, tzip, btbldgname, billingaddress, btcity, btstate, btzip, isonlinebill,textheader,demo, paymentterm, bldgnum, notabill)%>
  <table width="80%" border="0" cellpadding="0" cellspacing="0" align="center">
  <tr><td colspan="3" align="center"><font size="2"><% if textheader="advRealty.asp" then %>KEEP ABOVE PART FOR YOUR RECORDS<%end if%>&nbsp;</font></td></tr>
  <tr><td colspan="3" bgcolor="black" height="1"><img src="/images/spacer.gif" width="1" height="1"></td></tr>
  <tr><td colspan="3" align="center"><font size="2"><% if textheader="advRealty.asp" then %>DETACH HERE AND RETURN WITH PAYMENT<%end if%></font>&nbsp;<br></td></tr>
...
<%
	...
	%>
		<td width="50%" valign="top"><font size="1">Tenant Name and Address:</FONT><br>
	  <font size="3"><b><%=tenantname%> <% if tenantnum ="" or notabill = 1 then %> <%= "" %> <% else %> (<%=tenantnum%>) <% end if %> <br>
	  <%=replace(tstrt,vbNewLine,"<br>")%><br>
	  <%=tcity%>, <%=tstate%>&nbsp;<%=tzip%></b></font>
  </td>
    <% if trim(bldgnum) = "Pier57" then %>
		<td width="40%" valign="top"><font size="1">Make Check/Wire Payable To:</font><br>
			<font size="2">Acct Name&nbsp&nbsp&nbsp: RXR Pier 57 MT LLC</font><br>
			<font size="2">Bank&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp: JPMorgan Chase Bank N.A.</font><br>
			<font size="2">ABA Wires&nbsp&nbsp&nbsp: 021000021</font><br>
			<font size="2">ABA for ACH: 021000021</font><br>
			<font size="2">Account #&nbsp&nbsp&nbsp&nbsp&nbsp: 817992998</font><br>
			<font size="2">Swift Code&nbsp&nbsp&nbsp: CHASUS33</font><br>
			<font size="2">Notice must be sent to: arinquiry@rxrrealty.com</font><br>
		<% 
	else
		%>
			<td width="40%" valign="top"><font size="1">Make Check Payable To:</font><br>
		...
		<%
		%>
			<%=replace(rst2("billto"),vbcr,"<br>")%><br>
			<% if btcity="" and btstate="" then %><%=""%><% else %><%=rst2("btcity")%>, <%=rst2("btstate")%>&nbsp;<%=rst2("billzip")%><% end if %><br>
	<% end if %>
	  <% if notabill = 1 then %> <%=""%> <% else %> <%=paymentterm%> <% end if %></b><br></font>
	  </td>
		<% if textheader="advRealty.asp" then %><td width="10%" valign="top" align="right" nowrap><br><strong><font size="3"><u>TOTAL DUE: <%=FormatCurrency(masterTotal,2)%></u><font></strong><br></td><% end if %>
	<% end if%>
	</tr>
  ...
  <% 
	...
	if textheader<>"advRealty.asp" and tenantnum <> "" then %>
		<td colspan="3">&nbsp;<br><center><%=txtcontanct2%><br>&nbsp;<br><center><%=txtcontanct%><br>
					To view your online bill, login to tenant.genergy.com with access code</br><b><%=tenantnum%>.<%=building%></b>
		</center></td>
	<%end if%></tr>
	</table>
<%end sub
